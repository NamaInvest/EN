...
conn.on('ready', () => {
    console.log('✅ متصل - رفع نظام المنيو الإلكتروني الكامل...');
        ...
        stream.on('close', () => {
            conn.sftp((err, sftp) => {
                ...
                function startBuild() {
                    ...
                    conn.exec(`cd ${APP} && npm run build 2>&1 | tail -30 && pm2 restart saas-app && echo "✅ DONE"`, (err, stream2) => {
