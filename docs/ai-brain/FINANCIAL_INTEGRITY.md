...
17. **FX Gain/Loss Atomicity (Phase C.1):** Realized FX Gain/Loss MUST be computed inside `applyPayment` synchronously with the application. Fx processing MUST utilize a `FX_MATERIALITY_THRESHOLD` and utilize a lazy-load Fail-Fast strategy for missing settings (`FX_GAIN_GL_CODE`, `FX_LOSS_GL_CODE`). FX Journals MUST be correctly directed based on `partyType` (vendor/customer) and MUST be committed in the same `prisma.$transaction`.
18. **Outbox Pattern:** External API calls (like ZATCA) must NEVER be made synchronously inside a financial transaction.
