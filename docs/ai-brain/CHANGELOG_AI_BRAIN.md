# Changelog: AI Brain
## Version 1.1.28
- **Accounting Journal Contract Isolation (Phase 2.5.5):** Isolated the core accounting journal creation logic from `src/lib/auto-journal.ts` into a strictly-typed `AccountingJournalService`. By enforcing the `FinancialTxClient` parameter, this architectural shift secures the double-entry accounting engine against fragmented or partial commits. The legacy `createJournalEntry` function now acts as a wrapper that routes all calls through the new service contract, paving the way for complete route-centric logic deprecation.
- **Treasury Posting Service Contract (Phase 2.5.4):** Extracted treasury posting logic from `src/app/api/treasury/route.ts` into a strictly-typed `TreasuryPostingService`. The service enforces the use of `FinancialTxClient` to maintain strict atomicity between treasury entries and their associated journal entries, explicitly forbidding raw `PrismaClient` usage. The route now functions as a thin controller invoking `runFinancialTx`.
- **First Service Contract Migration (Phase 2.5.3):** Extracted the inventory stocktake approval logic from the API route (`api/inventory/stocktake/[id]/approve`) into a strongly-typed, isolated `InventoryAdjustmentService`. This service enforces a strict contract requiring an `InventoryTxClient`, explicitly rejecting raw `PrismaClient` instances. The API route was converted into a thin controller that invokes the service within `runInventoryTx`, marking the first successful transition towards a compile-time enforced Service Contract architecture.
- **Typed Transaction Helpers Foundation (Phase 2.5.2):** Established strongly typed wrappers (`runFinancialTx`, `runInventoryTx`) around `prisma.$transaction` in `src/lib/db/transaction.ts` using `Prisma.TransactionClient`. Updated `withTransaction` and `atomically` signatures to utilize `TxClient` rather than `any`, enforcing compile-time boundaries without introducing runtime breakages. This foundation allows future repository and service layers to natively mandate atomic transactions, further securing the financial core.
