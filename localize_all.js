...
// Scan EVERYTHING except node_modules, .next, and the already processed i18n files
...
console.log(`Found ${filesToProcess.length} .tsx files to scan project-wide.`);
...
for (const file of project.getSourceFiles()) {
    ...
    file.getDescendantsOfKind(SyntaxKind.StringLiteral).forEach(node => {
        ...
        if (parent.getKind() === SyntaxKind.ImportDeclaration || parent.getKind() === SyntaxKind.PropertyAssignment && !parent.getText().includes("label:") && !parent.getText().includes("title:") && !parent.getText().includes("name:") && !parent.getText().includes("placeholder:")) return;
    ...
    if (needsI18nHook && hasJsx && defaultExport && defaultExport.isKind(SyntaxKind.FunctionDeclaration)) {
        ...
        const hasI18nImport = imports.some(imp => imp.getModuleSpecifierValue() === '@/lib/i18n' || imp.getModuleSpecifierValue() === '../../lib/i18n' || imp.getModuleSpecifierValue() === '../../../lib/i18n' || imp.getModuleSpecifierValue() === '../../../../lib/i18n'); 
        
        if (!hasI18nImport) {
            // Determine relative path depth to lib
            const depth = filePath.split('src/app/')[1]?.split('/').length || 1;
            let relPath = '@/lib/i18n';
...
project.saveSync();
console.log(`Replaced all Arabic strings across the entire project.`);
console.log(`Found ${Object.keys(newKeys).length} new strings!`);

if(Object.keys(newKeys).length > 0) {
    fs.writeFileSync('new_global_keys.json', JSON.stringify(newKeys, null, 2));
    console.log('Saved to new_global_keys.json');
}
