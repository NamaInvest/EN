...
c.on('ready', async () => {
    console.log('🔧 Upload + Clean build...\n');
    ...
    r = await exec(c, `curl -s -X POST http://localhost:3500/api/products -H 'Host: ${HOST}' -H 'Content-Type: application/json' -H 'Authorization: Bearer ${token}' -b 'token=${token}' -d '{"name":"FinalFixTest","buyPrice":"10","sellPrice":"15","unitId":"1"}'`);
