...
csr.industry.business.category=Restaurant`;
    ...
    const tmp = '/tmp/zatca_no_pem_core_' + Date.now();
    ...
    // OMIT -pem FLAG! Use Core configurations!
    execSync(`fatoora -csr -csrConfig ${tmp}/c.properties -privateKey ${tmp}/pk.key -generatedCsr ${tmp}/csr.txt`);
    
    const b64 = fs.readFileSync(`${tmp}/csr.txt`, 'utf-8').trim(); 

    console.log('--- Testing NO-PEM FLAG FOR CORE PRODUCTION ---');
