...
async function run() {
  console.log('--- Phase 5: Transactional Document Sequences Deployment & Verification ---');
  
  // 1. Calculate Local SHA256
  console.log('\n[1/5] Calculating Local SHA256 Checksums...');
  const localHashes = {};
  for (const file of RUNTIME_FILES) {
    const fullPath = path.resolve('d:/namasoft9-3-main', file);
  ...
  conn.on('ready', async () => {
    try {
      ...
      console.log('\n🎉 --- PRODUCTION_DOCUMENT_SEQUENCE_DEPLOYED_AND_VERIFIED ---');
