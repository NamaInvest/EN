...
conn.on('ready', () => {
    console.log('✅ متصل - رفع ملفات خريطة المطعم إلى N11 (SaaS)...');
                ...
                files.forEach(f => {
                    ...
                    sftp.fastPut(localPath, `${APP}/${f}`, (err) => {
                        ...
                        if (done === files.length) {
                            console.log('\n⏳ إعادة بناء N11 (saas-app)... قد يستغرق دقيقة...');
                            conn.exec(`cd ${APP} && npx prisma generate && npm run build 2>&1 | tail -20 && pm2 restart saas-app && echo "✅ DONE"`, (err, stream2) => {
