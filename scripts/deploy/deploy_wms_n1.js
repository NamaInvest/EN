...
const fileTasks = [
    ...
    { local: 'c:/Users/1/Desktop/alfa/src/app/(dashboard)/smart-transfers/page.tsx', remote: '/src/app/(dashboard)/smart-transfers/page.tsx' },
    { local: 'c:/Users/1/Desktop/alfa/src/app/api/smart-transfers/route.ts', remote: '/src/app/api/smart-transfers/route.ts' }
];
...
async function rebuildServer(i) {
    return new Promise((resolve) => {
        ...
        conn.on('ready', async () => {
            console.log(`\n============================`);
            console.log(`[N${i}] Commencing Advanced WMS Deployment...`);
            ...
            conn.sftp(async (err, sftp) => {
                ...
                try {
                    console.log(`[N${i}] Uploading Transit Logic...`);
                    ...
                    console.log(`[N${i}] Files Synced. Triggering Next.js Build...`);
                    
                    const cmd = `cd ${basePath} && npm run build && pm2 restart n${i} || true`;
                    await execute(conn, cmd);
                    
                    console.log(`[N${i}] Done! WMS Advanced Transit Online.`);
                } catch (e) {
...
async function run() {
    console.log('Starting Phase 9.3 WMS In-Transit on N1...');
