...
conn.on('ready', () => {
    ...
    conn.exec('cat > /var/www/namasoft/remote_deploy.sh', (err, stream) => {
        ...
        conn.sftp((err, sftp) => {
            if (err) { console.error(err); process.exit(1); }
            sftp.fastPut('c:/Users/1/Desktop/alfa/src.zip', '/var/www/namasoft/src.zip', (e) => {
    ...
    host: hostIp, port: 22, username: 'root', privateKey: fs.readFileSync('C:/Users/1/.ssh/hetzner_key'), keepaliveInterval: 10000
});
