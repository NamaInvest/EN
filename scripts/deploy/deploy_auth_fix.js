...
const fileTasks = [
    { local: 'c:/Users/1/Desktop/alfa/src/lib/auth.ts', remote: '/src/lib/auth.ts' },
    { local: 'c:/Users/1/Desktop/alfa/src/app/api/users/route.ts', remote: '/src/app/api/users/route.ts' },
    { local: 'c:/Users/1/Desktop/alfa/src/app/api/auth/login/route.ts', remote: '/src/app/api/auth/login/route.ts' }
];
...
const runForServer = async (server) => {
    return new Promise((resolve) => {
        ...
        conn.on('ready', async () => {
            ...
            console.log(`[${server.name}] Patching Auth...`);
            conn.sftp(async (err, sftp) => {
                ...
                try {
                    ...
                    console.log(`[${server.name}] Auth Patched!`);
...
const runAll = async () => {
    console.log("🚀 Pushing Auth & Security Patch...");
    for (const s of SERVERS) await runForServer(s);
    console.log("✅ FINISHED Auth Patch!");
};
runAll();
