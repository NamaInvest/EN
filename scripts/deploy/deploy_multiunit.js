        ...
        conn.on('ready', () => {
            conn.sftp((err, sftp) => {
                ...
                const uploadNext = (idx) => {
                    if (idx >= FILES.length) {
                        sftp.end();
                        // migrate schema + build + restart
                        const cmd = `cd "${base}" && npx prisma db push --accept-data-loss 2>&1 | tail -3 && npm run build 2>&1 | tail -3 && pm2 restart ${node.pm2} && echo "✅ ${node.pm2} DONE"`;
                        conn.exec(cmd, (e, s) => {
...
async function main() {
    console.log('🚀 Deploying Multi-Unit System to N1-N11...\n');
