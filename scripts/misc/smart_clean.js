...
// 1. Strip the injected Phase 15 models entirely to reset
const markerIdx = lines.findIndex(l => l.includes('13.1 Purchase Requisitions'));
if (markerIdx !== -1) {
    // Keep everything BEFORE the line that has // ====================
    lines = lines.slice(0, markerIdx);
}

// 2. Remove any auto-generated or poorly-injected back relations across the ENTIRE file
for (let i = 0; i < lines.length; i++) {
    const l = lines[i];
    if (l.includes('PurchaseRequisition') && !l.includes('@@map')) {
        lines[i] = ''; 
    }
    if (l.includes('RequestForQuotation') && !l.includes('@@map')) {
        lines[i] = '';
    }
    if (l.includes('GoodsReceiptNote') && !l.includes('@@map')) {
        lines[i] = '';
    }
}

// Remove empty lines created by our blanking
lines = lines.filter(l => l !== '');

fs.writeFileSync('prisma/schema.prisma', lines.join('\\n'));
console.log("Sanitized schema.prisma");

// --- INJECT CORRECT RELATIONS ---
...
// Inject with proper explicit relationship names to satisfy Prisma 3-way bindings!
