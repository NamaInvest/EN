...
if (!hostIp) {
    console.error('Please provide IP');
    process.exit(1);
}
                ...
                while (active < limit && currentIndex < allFiles.length) {
                    ...
                    sftp.fastPut(path.resolve(file), `/var/www/namasoft/${file}`, (e) => {
                        ...
                        if (done + failed === allFiles.length) {
                            ...
                            const buildCmd = 'rm -f /tmp/build_sync.log && cd /var/www/namasoft && nohup bash -c "npx prisma generate && npm run build > /tmp/build_sync.log 2>&1 && pm2 restart namasoft" > /dev/null 2>&1 &';
...
}).connect({ host: hostIp, port: 22, username: 'root', password: 'VmJUML2LuezRSws', keepaliveInterval: 10000 });
