...
const arObjText = mappings.map(m => `        '${m.key}': '${m.ar}',`).join('\n');
const enObjText = mappings.map(m => `        '${m.key}': '${m.en}',`).join('\n');
...
const parts = i18n.split('export function I18nProvider');
i18n = parts[0] + injectionBlock + '\nexport function I18nProvider' + parts[1];
fs.writeFileSync('src/lib/i18n.tsx', i18n, 'utf8');
console.log('Successfully embedded AR/EN translations dynamically.');
