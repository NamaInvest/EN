...
const translateSidebar = (key) => {
    if (lang === 'en' && sidebarTranslationsEn[key]) return sidebarTranslationsEn[key];
    if (lang === 'ar' && sidebarTranslationsAr[key]) return sidebarTranslationsAr[key];
    return t(key);
};
`;

if (!code.includes('translateSidebar')) {
    code = code.replace('export default function Sidebar() {', 
`
const sidebarTranslationsEn = ${JSON.stringify(sidebarTranslationsEn, null, 4)};
const sidebarTranslationsAr = ${JSON.stringify(sidebarTranslationsAr, null, 4)};

export default function Sidebar() {`);

    code = code.replace('const { getSetting } = useSettings();', 
`const { getSetting } = useSettings();
${tLogic}
`);
}

// Replace t(group.sectionKey) and t(item.labelKey) safely
code = code.replace(/t\(group\.sectionKey\)/g, 'translateSidebar(group.sectionKey)');
code = code.replace(/t\(item\.labelKey\)/g, 'translateSidebar(item.labelKey)');
code = code.replace(/t\('sys\.str_101'\)/g, "(lang === 'en' ? 'Dashboard' : 'الرئيسية (Dashboard)')");

fs.writeFileSync('src/components/Sidebar.tsx', code, 'utf8');
console.log('Sidebar bypassed safely!');
