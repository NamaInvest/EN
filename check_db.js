...
conn.on('ready', () => {
    conn.exec('sudo -u postgres psql -h localhost -p 5432 -U postgres -d aljassim_db -c "SELECT key, value FROM \\"Setting\\" WHERE key LIKE \'%company%\' OR key = \'tax_number\';"', (err, stream) => {
        if (err) throw err;
