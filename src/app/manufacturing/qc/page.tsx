        ...
        <div className="min-h-screen p-6" dir="rtl">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-[var(--text-muted)] text-sm mt-1">فحص المنتج النهائي قبل الإرسال للمستودع</p>
                </div>
                <button className="px-5 py-2.5 btn-primary rounded-xl text-sm font-medium">+ سجل فحص جديد</button>
            </div>
                        ...
                        <button key={c.id} onClick={() => setSelected(c.id === selected ? null : c.id)}
                            ...
                            <div className="flex items-start justify-between">
                                <div>
                                    ...
                                    <p className="text-xs text-[var(--text-secondary)]">{c.batch} • مفتش: {c.inspector} • {c.date}</p>
                                </div>
                                <div className="text-left">
                                    <p className="text-sm text-[var(--text-muted)]">مفحوص: <span className="text-[var(--text)] font-bold">{c.checked}/{c.qty}</span></p>
