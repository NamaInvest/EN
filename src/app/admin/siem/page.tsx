        ...
        <div className="min-h-screen bg-gray-950 text-white p-6" dir="rtl">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">مراقبة الأحداث الأمنية وحماية بيانات المنشأة</p>
                </div>
                <div className="flex items-center gap-3">
                    <button onClick={() => setLive(!live)}
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-all ${live ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-gray-800 text-gray-400'}`}>
            ...
            {events.filter(e => e.severity === 'critical').map(e => (
                <div key={e.id} className="mb-3 p-3 bg-red-500/10 border border-red-500/30 rounded-xl flex items-center gap-3">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">{e.time} • {e.ip} • {e.user}</p>
                    </div>
                    <button className="text-xs px-3 py-1.5 bg-red-500 hover:bg-red-600 rounded-lg transition-colors">تحقيق</button>
                </div>
            ))}
            ...
            <div className="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden">
                <div className="p-3 border-b border-gray-800 flex items-center justify-between">
                    <span className="text-xs text-gray-500 font-mono">سجل الأحداث الأمني</span>
                ...
                <div className="overflow-auto max-h-96">
                    <table className="w-full text-xs font-mono">
                        ...
                        <tbody>
                            {filtered.map(e => (
                                <tr key={e.id} className="border-b border-gray-800/30 hover:bg-gray-800/30">
                                    <td className="p-2 text-gray-500">{e.time}</td>
                                    <td className="p-2 text-gray-400">{e.user}</td>
                                    <td className="p-2 text-gray-500">{e.ip}</td>
                                    <td className="p-2 text-gray-300 max-w-xs truncate">{e.msg}</td>
                                    <td className="p-2">
