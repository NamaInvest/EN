...
async function _GET(request: NextRequest) {
  ...
  try {
    const items = await (prisma as any).maintenanceWorkOrder.findMany({ take: 100, include: { schedule: true }, orderBy: { createdAt: 'desc' } });
    return NextResponse.json(items);
  } catch (e: any) {
 log.error('src/app/api/cmms/work-orders/route.ts', { error: e instanceof Error ? e.message : e });
 return apiError(e, 'Error', { context: 'cmms/work-orders' }); }
}
