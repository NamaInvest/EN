...
async function _GET(request: NextRequest) {
  ...
  try {
    const items = await (prisma as any).lmsCourse.findMany({ take: 100, include: { _count: { select: { modules: true, enrollments: true } } }, orderBy: { createdAt: 'desc' } });
    return NextResponse.json(items);
  } catch (e: any) {
 log.error('src/app/api/lms/courses/route.ts', { error: e instanceof Error ? e.message : e });
 return apiError(e, 'Error', { context: 'lms/courses' }); }
}
