...
export const GET = withGuard(async (req, params, user) => {
    ...
    const saleRefs = completedSales.map(i => `SALE-${i.invoiceNo}`);
    const saleJournals = await prisma.journalEntry.findMany({
        where: { reference: { in: saleRefs }, status: 'posted' },
        select: { reference: true }
    });
    const saleJournalSet = new Set(saleJournals.map(j => j.reference));
    ...
    const purRefs = completedPurchases.map(i => `PUR-${i.invoiceNo}`);
    const purJournals = await prisma.journalEntry.findMany({
        where: { reference: { in: purRefs }, status: 'posted' },
        select: { reference: true }
    });
    const purJournalSet = new Set(purJournals.map(j => j.reference));
