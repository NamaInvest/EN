...
async function _GET(request: NextRequest) {
    ...
    try {
        const vacations = await prisma.vacation.findMany({ 
            take: 100, 
            include: { 
                employee: { select: { id: true, name: true, position: true, phone: true } } 
            }, 
            orderBy: { id: 'desc' } 
        });
...
export const POST = withRoute(async ({ req }) => _POST(req), { rateLimit: 'DEFAULT' });

export const PUT = withRoute(async ({ req }) => _PUT(req), { rateLimit: 'DEFAULT' });
