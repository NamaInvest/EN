...
async function _GET(req: NextRequest) {
    ...
    try {
        ...
        // Assets
        const customers = await prisma.customer.findMany({ take: 100, select: { balance: true } });
        const totalAR = customers.reduce((sum: any, c: any) => sum + (c.balance || 0), 0);
        
        const products = await prisma.product.findMany({ take: 100, select: { currentStock: true, buyPrice: true } });
        const totalInventory = products.reduce((sum: any, p: any) => sum + ((p.currentStock || 0) * (p.buyPrice || 0)), 0);

        const treasuries = await prisma.treasury.findMany({ take: 100, select: { amount: true, type: true } });
        let totalCash = 0;
        ...
        // Liabilities
        const suppliers = await prisma.customer.findMany({ take: 100, where: { type: 1 }, select: { balance: true } }); // type 1 = supplier
        ...
        const sales = await prisma.salesInvoice.findMany({ take: 100, select: { total: true, date: true } });
        const totalSales = sales.reduce((sum: any, s: any) => sum + s.total, 0);

        const expenses = await prisma.expense.findMany({ take: 100, select: { amount: true } });
