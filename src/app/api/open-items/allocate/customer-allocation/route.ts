...
const RealCustomerSchema = z.object({
  partnerId: z.number(),
  treasuryId: z.number(),
...
async function _POST(ctx: any) {
  ...
  try {
    ...
    const parsed = RealCustomerSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const matches = await runFinancialTx(prisma, async (tx) => {
      ...
      for (const alloc of allocations) {
        const match = await OpenItemsService.allocateCustomerPayment(tx, {
          tenantId: tenant,
    ...
    log.error('Customer payment allocation execution error', { error: error.message, tenant });
