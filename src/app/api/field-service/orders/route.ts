...
const log = logger.child({ service: 'field-service.orders' });
async function _GET(request: NextRequest) {
  const prisma = getPrisma(request as any);
  try {
    const items = await (prisma as any).fieldServiceOrder.findMany({ take: 100, orderBy: { scheduledDate: 'desc' } });
    return NextResponse.json(items);
  } catch (e: any) {
 log.error('src/app/api/field-service/orders/route.ts', { error: e instanceof Error ? e.message : e });
 return apiError(e, 'Error', { context: 'field-service/orders' }); }
}
