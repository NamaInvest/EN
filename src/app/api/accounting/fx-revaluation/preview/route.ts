...
const log = logger.child({ service: 'fx-revaluation-preview-route' });
...
async function _POST(ctx: any) {
  ...
  try {
    ...
    // Calculate the dry-run revaluation preview using our read-only engine method
    const preview = await FXRevaluationEngine.previewARAP(prisma, tenant, targetDate);
    ...
    log.error('FX revaluation dry-run preview execution error', { error: error.message, tenant });
