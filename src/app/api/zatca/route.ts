        ...
        if (action === 'onboard') {
            ...
            try {
                ...
                if (response.status === 200 && response.data?.binarySecurityToken) {
                    ...
                    // Sync private key + compliance token to main settings table for QR route
                    await prisma.setting.upsert({ where: { key: 'zatca_private_key' }, update: { value: egsData.private_key }, create: { key: 'zatca_private_key', value: egsData.private_key, description: 'ZATCA Private Key' } });
                    await prisma.setting.upsert({ where: { key: 'zatca_compliance_token' }, update: { value: response.data.binarySecurityToken }, create: { key: 'zatca_compliance_token', value: response.data.binarySecurityToken, description: 'ZATCA Compliance Token' } });
        ...
        if (action === 'production-csid') {
            ...
            try {
                ...
                if (response.status === 200 && response.data?.binarySecurityToken) {
                    ...
                    // Sync production tokens + private key to main settings table for QR route & frontend
                    await prisma.setting.upsert({ where: { key: 'zatca_production_token' }, update: { value: response.data.binarySecurityToken }, create: { key: 'zatca_production_token', value: response.data.binarySecurityToken, description: 'ZATCA Production Token' } });
                    await prisma.setting.upsert({ where: { key: 'zatca_production_secret' }, update: { value: response.data.secret || '' }, create: { key: 'zatca_production_secret', value: response.data.secret || '', description: 'ZATCA Production Secret' } });
                    await prisma.setting.upsert({ where: { key: 'zatca_private_key' }, update: { value: settings.private_key || '' }, create: { key: 'zatca_private_key', value: settings.private_key || '', description: 'ZATCA Private Key' } });
