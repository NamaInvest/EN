...
export const POST = withRoute(async ({ req, prisma, auth, tenant }) => {
  ...
  if (body.type === 'classify') {
    const result = await SpendAnalyticsEngine.classify(tenant, body.transactionType, body.transactionId, body.description, body.categoryId);
...
export const GET = withRoute(async ({ req, prisma, auth, tenant }) => {
  const cube = await SpendAnalyticsEngine.buildCube(tenant);
