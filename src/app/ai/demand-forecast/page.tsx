        ...
        <div className="min-h-screen bg-gray-950 text-white p-6" dir="rtl">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">تنبؤ باحتياجات المخزون باستخدام Moving Average + Trend Detection</p>
                </div>
            </div>
                ...
                <div className="flex gap-2 mb-0">
                    {[['batch','📊 تنبؤ جماعي (أفضل 20 منتج)'],['single','🔍 تنبؤ منتج محدد']].map(([k,l]) => (
                        <button key={k} onClick={() => { setMode(k as any); setResults(null); }}
                            className={`px-4 py-2 rounded-lg text-sm transition-all ${mode===k?'bg-blue-500 text-white':'bg-gray-800 text-gray-400 hover:text-white'}`}>{l}</button>
                    ))}
                </div>
                {mode === 'single' && (
                    <input value={productId} onChange={e => setProductId(e.target.value)} placeholder="أدخل رقم المنتج..."
                        className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm w-48 focus:outline-none focus:border-blue-500" />
                )}
            ...
            {!results && !loading && (
                <div className="bg-gray-900 rounded-2xl border border-gray-800 p-8 text-center">
                    ...
                    <p className="text-gray-400">اضغط "تشغيل التنبؤ" لتحليل بيانات المبيعات وتوقع الطلب</p>
                    <div className="grid md:grid-cols-3 gap-4 mt-6 text-sm text-gray-500">
                        {['📈 يحلل مبيعات 90 يوم الماضية','📊 يكتشف الاتجاه (صاعد/هابط/مستقر)','📦 يوصي بكمية الطلب المثلى'].map(t => (
                            <div key={t} className="bg-gray-800 rounded-xl p-3">{t}</div>
                        ))}
                    </div>
                </div>
            )}
            ...
            {loading && (
                <div className="text-center py-20">
                    ...
                    <p className="text-gray-400">يحلل النظام بيانات المبيعات...</p>
                </div>
            )}
                            ...
                            <tbody>
                                {(results.forecasts || []).map((f: any, i: number) => (
                                    <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                                        ...
                                        <td className="p-3 text-gray-300">{f.currentStock}</td>
                ...
                <div className="space-y-4">
                    ...
                    {results.forecast?.recommendedOrder > 0 && (
                        <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
                            <p className="text-amber-400 font-medium">📋 كمية الطلب الموصى بها: <strong className="text-white text-lg">{results.forecast?.recommendedOrder} وحدة</strong></p>
                        </div>
                    )}
                </div>
            )}
