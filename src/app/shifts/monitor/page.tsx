...
const STATUS_CFG: Record<string, { label: string; dot: string; card: string }> = {
    ...
    idle: { label: '⚪ خامل', dot: 'bg-gray-400', card: 'border-gray-700 bg-gray-900' },
        ...
        <div className="min-h-screen bg-gray-950 text-white p-6" dir="rtl">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">لوحة مشرف الكاشير — جميع الكاشيرات في الوقت الفعلي</p>
                </div>
                    ...
                    <div key={c.id} className={`rounded-2xl border p-5 ${STATUS_CFG[c.status]?.card}`}>
                        <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs text-gray-400">{STATUS_CFG[c.status]?.label}</span>
                            </div>
                            <span className="text-xs font-mono text-gray-500">{c.register}</span>
                        </div>
                        <h3 className="font-semibold text-white mb-3">{c.name}</h3>
                        <div className="space-y-2 text-sm">
                            ...
                            <div className="flex justify-between">
                                <span className="text-gray-400">الفواتير</span>
                                <span className="text-white">{c.invoices}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-400">نقد / شبكة</span>
                                <span className="text-gray-300 text-xs">{c.cash.toLocaleString()} / {c.card.toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-400">آخر معاملة</span>
                                <span className={`text-xs ${c.lastTx === 'الآن' ? 'text-emerald-400' : 'text-gray-400'}`}>{c.lastTx}</span>
                            </div>
                        </div>
            ...
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4">
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-sm font-semibold text-gray-300">آخر المعاملات — مباشر</h3>
                </div>
                <div className="space-y-2">
                    {[
                        ...
                        <div key={i} className="flex items-center justify-between text-sm py-1.5 border-b border-gray-800/50 last:border-0">
                            <span className="font-mono text-gray-500 text-xs">{tx.time}</span>
                            <span className="text-gray-400">{tx.cashier}</span>
                            ...
                            <span className="text-white font-bold">{tx.amount.toLocaleString()} ر.س</span>
                        </div>
                    ))}
                </div>
            </div>
