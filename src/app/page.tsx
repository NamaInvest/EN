      ...
      <nav className="sticky top-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
           ...
           <div className="flex gap-4">
              <button onClick={() => window.location.href = '/sign-in'} className="text-slate-600 font-bold text-sm hidden sm:block hover:text-indigo-600">تسجيل الدخول</button>
              <button onClick={() => window.location.href = '/sign-up'} className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg shadow-md shadow-indigo-600/30 transition-all">إنشاء حساب جديد</button>
           </div>
        </div>
      </nav>
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: '50px' }}>
          ...
          <div style={{ flex: '1 1 40%', minWidth: '300px' }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto mt-6">
                <button onClick={() => window.location.href = '/sign-up'} className="px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-600/30 transition-all flex items-center justify-center gap-2 text-lg">
