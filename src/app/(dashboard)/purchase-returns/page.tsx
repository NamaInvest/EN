...
export default function PurchaseReturnsPage() {
    ...
    return (<><div className="page-header"><h1 className="page-title">🔄 مرتجعات المشتريات</h1></div>
        <div className="page-content animate-fade-in">
            ...
            {showAdd && <div className="card" style={{ marginBottom: '16px', padding: '16px', display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div><label style={{ fontSize: '12px', display: 'block', marginBottom: '4px' }}>رقم فاتورة الشراء</label><input value={form.originalInvoiceId} onChange={e => setForm({ ...form, originalInvoiceId: e.target.value })} style={{ width: '120px', padding: '6px 10px', borderRadius: '6px', border: '1px solid var(--border)' }} /></div>
            ...
            <div className="card">
                {loading ? <div className="empty-state"><div className="empty-state-text">جاري التحميل...</div></div> :
                    returns.length === 0 ? <div className="empty-state"><div className="empty-state-icon">🔄</div><div className="empty-state-text">لا توجد مرتجعات مشتريات</div></div> :
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead><tr style={{ background: 'rgba(108,99,255,0.05)' }}><th style={{ padding: '8px', textAlign: 'right' }}>#</th><th style={{ padding: '8px', textAlign: 'right' }}>التاريخ</th><th style={{ padding: '8px', textAlign: 'right' }}>المبلغ</th><th style={{ padding: '8px', textAlign: 'right' }}>الضريبة</th><th style={{ padding: '8px', textAlign: 'right' }}>الإجمالي</th></tr></thead>
                            <tbody>{returns.map(r => (
