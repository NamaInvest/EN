...
export default function StockTransfersPage() {
 ...
 <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>{transfers.map((tr: any) => (
 ...
 {expanded === tr.id && tr.details && <table style={{ width: '100%', marginTop: '10px', borderCollapse: 'collapse' }}>
 ...
 <tbody>{tr.details.map((d: any, i: number) => <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}><td style={{ padding: '6px', fontSize: '12px' }}>{d.productName}</td><td style={{ padding: '6px', textAlign: 'center', fontFamily: 'monospace' }}>{d.quantity}</td></tr>)}</tbody>
 </table>}
 </div>
 ))}</div>}
 </div>
