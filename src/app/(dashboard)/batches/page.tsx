...
const batchSchema = z.object({
  ...
  batchNumber: z.string().min(1, 'رقم التشغيلة مطلوب').toUpperCase(),
  productionDate: z.string().optional(),
  expiryDate: z.string().optional(),
  initialQuantity: z.coerce.number().min(0, 'الكمية يجب أن تكون موجبة').optional(),
  unitCost: z.coerce.number().min(0, 'التكلفة يجب أن تكون موجبة').optional(),
});
...
export default function BatchesPage() {
  ...
  const handleSave = async (data: BatchFormValues) => {
    setSaving(true);
    try {
      ...
      if (res.ok) { 
        setShowModal(false); 
        toastSuccess('تم حفظ التشغيلة بنجاح');
        fetchData(); 
      } else { 
  ...
  const defaultValues: Partial<BatchFormValues> = editItem ? {
    productId: editItem.productId,
    batchNumber: editItem.batchNumber,
    productionDate: editItem.productionDate ? new Date(editItem.productionDate).toISOString().split('T')[0] : '',
    expiryDate: editItem.expiryDate ? new Date(editItem.expiryDate).toISOString().split('T')[0] : '',
    initialQuantity: editItem.initialQuantity,
    unitCost: editItem.unitCost
  } : {
    productId: undefined, batchNumber: '', productionDate: '', expiryDate: '', initialQuantity: 0, unitCost: 0
  };
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '600px' }}>
            ...
            <Form schema={batchSchema} defaultValues={defaultValues} onSubmit={handleSave} className="space-y-4 p-4">
              {!editItem && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormSelect 
                  ...
                  label={t('sys.str_438')} 
                  options={[
                    { label: t('sys.str_439'), value: '' },
                    ...products.map(p => ({ label: `${p.name} ${p.barcode ? `(${p.barcode})` : ''}`, value: p.id.toString() }))
                  ]}
                  disabled={!!editItem}
                />
                <FormField 
              ...
              {!editItem && (
                <FormField name="initialQuantity" type="number" label={t('sys.str_441')} placeholder={t('sys.str_453')} dir="ltr" step="0.01" />
              )}

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField name="productionDate" type="date" label={t('sys.str_442')} dir="ltr" />
                <FormField name="expiryDate" type="date" label={t('sys.str_443')} dir="ltr" />
              </div>

              <FormField name="unitCost" type="number" label={t('sys.str_444')} dir="ltr" step="0.01" />
