...
export default function ConsolidationDashboard() {
 ...
 <p className="text-slate-500 mt-1 text-sm">{_t('IFRS 10 / IAS 21 — توحيد القوائم وإزالة العمليات بين الشركات', 'IFRS 10 / IAS 21 — Group consolidation & IC eliminations')}</p>
 </div>
 <div className="flex gap-2">
 <select value={selectedGroup} onChange={e => setSelectedGroup(e.target.value)} className="px-3 py-2 border rounded-md text-sm ">
 ...
 <h2 className="text-lg font-medium text-slate-900 ">{_t('ملخص التوحيد', 'Run Summary')} #{summary.runId}</h2>
 <button onClick={() => setSummary(null)} className="text-slate-400 hover:text-slate-600">✕</button>
 </div>
 <div className="grid grid-cols-4 gap-4">
 <div className="p-3 bg-slate-50 rounded"><p className="text-xs text-slate-500">{_t('الحالة','Status')}</p><p className="font-bold">{summary.status}</p></div>
 <div className="p-3 bg-slate-50 rounded"><p className="text-xs text-slate-500">{_t('الإزالات','Eliminations')}</p><p className="font-bold font-mono">{summary.totalEliminations} ({fmt(summary.totalEliminationAmount)})</p></div>
 <div className="p-3 bg-slate-50 rounded"><p className="text-xs text-slate-500">{_t('فروقات العملة','CTA')}</p><p className="font-bold font-mono">{fmt(summary.ctaAmount)}</p></div>
 <div className="p-3 bg-slate-50 rounded"><p className="text-xs text-slate-500">{_t('حقوق الأقلية','NCI')}</p><p className="font-bold font-mono">{fmt(summary.nciAmount)}</p></div>
 </div>
 </div>
 )}
 ...
 <h2 className="text-lg font-medium text-slate-900 ">{_t('عمليات التوحيد', 'Consolidation Runs')}</h2>
 </div>
 ...
 runs.length===0?<tr><td colSpan={5} className="text-center py-10 text-slate-500">{_t('لا توجد عمليات','No runs yet')}</td></tr>:
 runs.map((r:any)=>(
