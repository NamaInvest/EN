...
export default function BudgetControlDashboard() {
 ...
 <p className="text-slate-500 mt-1 text-sm">{_t('مراقبة الصرف وتحليل الانحرافات والارتباطات', 'Spending control, variance analysis & encumbrance tracking')}</p>
 </div>
 <div className="flex gap-2">
 <select value={year} onChange={e => setYear(parseInt(e.target.value))} className="px-3 py-2 border rounded-md text-sm ">
 ...
 <h2 className="text-lg font-medium text-slate-900 mb-4">{_t('فحص التوفر', 'Budget Availability Check')}</h2>
 <div className="flex gap-3 items-end">
 <div>
 <label className="block text-sm text-slate-500 mb-1">{_t('رقم الحساب', 'Account ID')}</label>
 <input type="number" value={checkAccountId} onChange={e => setCheckAccountId(e.target.value)} className="px-3 py-2 border rounded-md text-sm "placeholder="5100" />
 </div>
 <div>
 <label className="block text-sm text-slate-500 mb-1">{_t('المبلغ', 'Amount')}</label>
 <input type="number" value={checkAmount} onChange={e => setCheckAmount(e.target.value)} className="px-3 py-2 border rounded-md text-sm "placeholder="10000" />
 </div>
 ...
 <h2 className="text-lg font-medium text-slate-900 ">{_t('الميزانيات', 'Budgets')} — {year}</h2>
 </div>
 ...
 {loading?<tr><td colSpan={4} className="text-center py-10 text-slate-500">{_t('جاري التحميل...','Loading...')}</td></tr>:
 budgets.length===0?<tr><td colSpan={4} className="text-center py-10 text-slate-500">{_t('لا توجد ميزانيات','No budgets')}</td></tr>:
 budgets.map((b:any)=>(
 <tr key={b.id} className="hover:bg-slate-50 dark:hover:bg-gray-700/50">
