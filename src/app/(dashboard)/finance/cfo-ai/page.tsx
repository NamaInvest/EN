...
export default function CFOAIPage() {
 ...
 const columns: ColumnDef<any>[] = [
  ...
  { accessorKey: 'currentPrice', header: 'السعر الحالي', cell: (info) => `${info.getValue()} SAR` },
  {
   accessorKey: 'recommendedPrice',
   header: 'السعر المقترح (AI)',
   cell: ({ row }) => {
    const recPrice = row.original.recommendedPrice;
    const curPrice = row.original.currentPrice;
    return (
     <span className={recPrice > curPrice ? 'text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded' : 'text-rose-400 bg-rose-500/10 px-2 py-1 rounded'}>
      {recPrice} SAR
     </span>
    );
   }
  },
  { accessorKey: 'reason', header: 'السبب المالي', cell: (info) => <div className="text-slate-400 text-xs max-w-xs">{String(info.getValue())}</div> },
  {
   id: 'actions',
   header: 'إجراء',
   cell: () => (
    <button 
      onClick={() => info(_t('التحديث التلقائي للأسعار قيد التطوير', 'Dynamic pricing update is under development'))}
      className="text-xs bg-indigo-500 hover:bg-indigo-600 text-white px-3 py-1.5 rounded-lg transition-colors flex items-center shadow-lg shadow-indigo-500/20"
    >
 ...
 <div className="overflow-x-auto">
  <DataTable columns={columns} data={data?.pricingRecommendations || []} />
 </div>
 </div>
 </div>
 ...
 <button 
    onClick={() => info(_t('تم إرسال طلب التجميد للموافقة الإدارية', 'Freeze request sent for administrative approval'))}
    className="mt-3 w-full py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs font-medium border border-slate-700 transition-colors"
  >
  تفعيل التجميد الآلي
  </button>
 </div>
