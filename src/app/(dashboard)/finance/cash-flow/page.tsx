        ...
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 text-sm">{_t('تحليل تنبئي للسيولة وفق IAS 7', 'Predictive liquidity per IAS 7')}</p>
                </div>
                <div className="flex gap-2 flex-wrap">
                    <select value={period} onChange={e => setPeriod(e.target.value)} className="px-3 py-2 border rounded-md text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                    ...
                    <select value={scenario} onChange={e => setScenario(e.target.value)} className="px-3 py-2 border rounded-md text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                    ...
                    <button onClick={compareScenarios} className="px-4 py-2 border rounded-md text-sm font-medium text-gray-700 dark:text-white bg-white dark:bg-gray-800 hover:bg-gray-50 flex items-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                {[
                    ...
                    <div key={i} className={`p-5 rounded-lg border shadow-sm ${c.cls || 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700'}`}>
                        <p className="text-sm font-medium text-gray-500 mb-1 flex items-center">{c.icon}{c.label}</p>
                        <h3 className={`text-2xl font-bold font-mono ${c.color || 'text-gray-900 dark:text-white'}`}>{c.value}</h3>
                    </div>
                ))}
            </div>
            ...
            {alerts.length > 0 && <div className="space-y-2">{alerts.map((a: any, i: number) => (
                <div key={i} className={`p-4 rounded-lg border flex items-center gap-3 ${a.severity==='HIGH'?'bg-red-50 dark:bg-red-900/20 border-red-300':'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-300'}`}>
                    ...
                    <span className="text-sm font-medium text-gray-900 dark:text-white">{a.message}</span>
                </div>
            ))}</div>}

            {comparison && (
                <div className="bg-white dark:bg-gray-800 border rounded-lg shadow-sm p-6">
                    <h2 className="text-lg font-medium text-gray-900 dark:text-white mb-4">{_t('مقارنة السيناريوهات', 'Scenario Comparison')}</h2>
                    ...
                    <p className="text-sm bg-gray-100 dark:bg-gray-700 p-3 rounded"><strong>{_t('التوصية:','Rec:')}</strong> {comparison.recommendation}</p>
                </div>
            )}
            ...
            <div className="bg-white dark:bg-gray-800 border rounded-lg shadow-sm">
                <div className="p-4 border-b bg-gray-50 dark:bg-gray-900/50">
                    <h2 className="text-lg font-medium text-gray-900 dark:text-white">{_t('تفاصيل الفترات', 'Period Breakdown')}</h2>
                </div>
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            buckets.length===0?<tr><td colSpan={5} className="text-center py-10 text-gray-500">{_t('اضغط إنشاء تنبؤ','Click Generate')}</td></tr>:
                            buckets.map((b:any,i:number)=>(
                                <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{b.label}</td>
