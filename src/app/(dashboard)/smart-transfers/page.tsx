...
const formSchema = z.object({
  selectedProduct: z.string().min(1, 'Product is required'),
  senderStock: z.string().min(1, 'Sender stock is required'),
  receiverStock: z.string().min(1, 'Receiver stock is required'),
...
export default function SmartTransfersPage() {
  ...
  const handleDispatch = async (data: FormValues) => {
    if (data.senderStock === data.receiverStock) return toastWarning(t('sys.str_1435'));
    ...
    try {
      const res = await fetch('/api/smart-transfers', {
        ...
        body: JSON.stringify({
          ...
          senderStockId: data.senderStock,
          receiverStockId: data.receiverStock,
      ...
      if (resData.success) {
        ...
        toastError(resData.error || t('sys.str_1436'));
      }
    } catch (e) {
      toastError(t('sys.str_1437'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-6 max-w-6xl mx-auto" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div className="flex border-b border-divider mb-8">
          <button 
            ...
            className={`px-6 py-3 font-semibold text-lg flex items-center gap-2 border-b-2 transition ${activeTab === 'dispatch' ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-500'}`}
          >
            <Send size={20} /> {t('sys.str_1414')}</button>
          <button 
            onClick={() => setActiveTab('track')}
            className={`px-6 py-3 font-semibold text-lg flex items-center gap-2 border-b-2 transition ${activeTab === 'track' ? 'border-orange-500 text-orange-400' : 'border-transparent text-slate-500'}`}
          >
        ...
        {activeTab === 'dispatch' && (
          <div className="bg-surface border border-divider rounded-2xl p-8 shadow-sm">
            ...
            <form onSubmit={handleSubmit(handleDispatch)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-gray-300 font-bold mb-2">{t('sys.str_1419')}</label>
                  <select 
                    className={`w-full bg-[#111] border border-divider text-white p-3 rounded-xl outline-none focus:border-blue-500 ${errors.selectedProduct ? 'border-red-500' : ''}`}
                  ...
                  {errors.selectedProduct && <span className="text-red-500 text-xs mt-1">{errors.selectedProduct.message}</span>}
                </div>
                <div>
                  ...
                  <input 
                    ...
                    className={`w-full bg-[#111] border border-divider text-white p-3 rounded-xl outline-none focus:border-blue-500 ${errors.quantity ? 'border-red-500' : ''}`}
                ...
                <div className="p-4 bg-black/20 rounded-xl border border-dashed border-gray-600">
                  <label className="block text-slate-400 font-bold mb-2 text-sm">{t('sys.str_1421')}</label>
                  <select 
                    className={`w-full bg-transparent text-white p-2 border-b border-gray-700 outline-none focus:border-blue-500 ${errors.senderStock ? 'border-red-500' : ''}`}
                    {...register('senderStock')}
                  >
                  ...
                  {errors.senderStock && <span className="text-red-500 text-xs mt-1">{errors.senderStock.message}</span>}
                </div>
                <div className="p-4 bg-black/20 rounded-xl border border-dashed border-gray-600">
                  <label className="block text-orange-400 font-bold mb-2 text-sm">{t('sys.str_1423')}</label>
                  <select 
                    className={`w-full bg-transparent text-white p-2 border-b border-gray-700 outline-none focus:border-orange-500 ${errors.receiverStock ? 'border-red-500' : ''}`}
                    {...register('receiverStock')}
                  >
                  ...
                  {errors.receiverStock && <span className="text-red-500 text-xs mt-1">{errors.receiverStock.message}</span>}
                </div>
              </div>
          ...
          <div className="space-y-6">
              ...
              <div className="p-0 overflow-x-auto">
                <table className="w-full text-left" dir={isRTL ? 'rtl':'ltr'}>
                  <thead className="bg-[#111]">
                    <tr>
                      <th className="p-4 font-medium text-slate-400">{t('sys.str_1426')}</th>
                      <th className="p-4 font-medium text-slate-400">{t('sys.str_63')}</th>
                      <th className="p-4 font-medium text-slate-400">{t('sys.str_64')}</th>
                      <th className="p-4 font-medium text-slate-400">{t('sys.str_1427')}</th>
                      <th className="p-4 font-medium text-slate-400">{t('sys.str_1428')}</th>
                      <th className="p-4 font-medium text-slate-400">{t('fin.str_232')}</th>
                      <th className="p-4 font-medium text-slate-400">{t('sys.str_1429')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {activeTransits.map(tr => (
                      <tr key={tr.id} className="border-t border-divider hover:bg-white/5 transition">
                        ...
                        <td className="p-4 text-slate-400">{tr.senderStock}<br/><span className="text-xs">{t('sys.str_1430')}{tr.senderName}</span></td>
                        <td className="p-4 text-orange-400 font-bold border-l-2 border-r-2 border-dashed border-orange-500/30">{tr.receiverStock}</td>
                        <td className="p-4 text-sm text-slate-400">{new Date(tr.date).toLocaleDateString('en-GB')}</td>
                        <td className="p-4">
                          <button 
                            disabled={loading}
                    ...
                    {activeTransits.length === 0 && (
                      <tr><td colSpan={7} className="p-8 text-center text-slate-500">{t('sys.str_1432')}</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <div className="bg-surface rounded-2xl border border-divider overflow-hidden opacity-80">
              <div className="p-4 bg-[#111] border-b border-divider">
                <h2 className="text-lg font-bold text-slate-400 flex items-center gap-2"><PackageCheck /> {t('sys.str_1433')}</h2>
              </div>
                  ...
                  <tbody>
                    {completedTransits.map(tr => (
                      <tr key={tr.id} className="border-b border-divider/50">
                        ...
                        <td className="p-4 text-slate-400">{tr.senderStock} ➔ {tr.receiverStock}</td>
