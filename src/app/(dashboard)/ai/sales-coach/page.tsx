      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="card p-5 border border-[var(--border)] bg-[var(--bg-secondary)] rounded-xl shadow-sm text-center">
          ...
          <div className="card p-6 border border-[var(--border)] bg-[var(--bg-secondary)] rounded-xl shadow-sm">
            ...
            <ul className="space-y-3">
              {data.recommendations?.map((r: string, i: number) => (
                <li key={i} className="p-4 bg-[var(--bg-primary)] rounded-lg border border-[var(--border)] shadow-sm text-[var(--text)]">
                  {r}
                </li>
              ))}
            </ul>
