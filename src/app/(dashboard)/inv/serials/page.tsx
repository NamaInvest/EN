  ...
  return (
    <div className="p-6" dir="rtl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">📦 المخزون - الأرقام التسلسلية</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        {loading ? (
          <div className="p-8 text-center text-slate-500">جاري تحميل بيانات التتبع...</div>
        ) : serials.length === 0 ? (
          <div className="p-8 text-center text-slate-500">لـم يتم العثور على أي أصناف مسجلة برقم تسلسلي.</div>
        ) : (
                ...
                <tr key={i} className="border-b border-slate-100 hover:bg-slate-50 transition">
                  ...
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${
                      ...
                      {sn.status === 'IN_STOCK' ? '🟢 متوفر في المخزون' : sn.status === 'SOLD' ? '⚪ مباع' : sn.status}
                    </span>
                  </td>
