...
export default function ZakatDashboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!res.ok) throw new Error(lang === 'ar' ? 'فشل جلب بيانات الزكاة' : 'Failed to fetch Zakat data');
        ...
        setData({
          ...
          adjustments: Array.isArray(json) ? json : json.adjustments || [
            { desc: _t('إضافات على صافي الربح (مخصصات)', 'Additions to net profit (provisions)'), amount: 150000, type: 'add' },
            { desc: _t('خصومات من صافي الربح (إيرادات مستثناة)', 'Deductions from net profit (exempt revenues)'), amount: -50000, type: 'deduct' },
            { desc: _t('الإضافات على الوعاء الزكوي (رأس المال)', 'Additions to Zakat base (capital)'), amount: 2000000, type: 'add' },
            { desc: _t('الخصومات من الوعاء (أصول ثابتة)', 'Deductions from base (fixed assets)'), amount: -800000, type: 'deduct' }
          ],
          schedules: json.schedules || [
            { name: _t('إقرار الأصول الثابتة', 'Fixed Assets Schedule'), status: 'ready' },
            { name: _t('إقرار المخصصات والاحتياطيات', 'Provisions & Reserves Schedule'), status: 'ready' },
            { name: _t('حركة حقوق الملكية', 'Statement of Changes in Equity'), status: 'pending' }
          ]
        });
      } catch (err: any) {
  ...
  const formatCurrency = (amount: number) => {
    return new Intl.NumberFormat(lang === 'ar' ? 'ar-SA' : 'en-US', { style: 'currency', currency: 'SAR' }).format(Math.abs(amount));
  };
  ...
  if (loading) {
    return (
      <div style={{ textAlign: 'center', padding: '5rem', color: 'var(--text-muted)' }}>
        ...
        <p>{_t('جاري تحميل الإقرار الزكوي...', 'Loading Zakat Declaration...')}</p>
      </div>
    );
  }
    ...
    <div className="page-content" style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto' }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h1 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          {_t('الإقرار الزكوي (Zakat Declaration)', 'Zakat Declaration Dashboard')}
        </h1>
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'var(--bg-secondary)', padding: '0.5rem 1rem', borderRadius: '8px' }}>
            ...
            >
              <option value="2026">{_t('سنة الإقرار 2026', 'Declaration Year 2026')}</option>
              <option value="2025">{_t('سنة الإقرار 2025', 'Declaration Year 2025')}</option>
              <option value="2024">{_t('سنة الإقرار 2024', 'Declaration Year 2024')}</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '1.5rem', [lang === 'ar' ? 'borderRight' : 'borderLeft']: '4px solid #3b82f6' }}>
          <div style={{ padding: '1rem', background: '#3b82f620', borderRadius: '12px' }}>
            <ScrollText size={32} color="#3b82f6" />
          </div>
          <div>
            <div style={{ fontSize: '0.9rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>{_t('صافي الربح المحاسبي (المعدل)', 'Adjusted Net Accounting Profit')}</div>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#3b82f6' }}>{formatCurrency(data?.netIncome || 0)}</div>
          </div>
        </div>

        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '1.5rem', [lang === 'ar' ? 'borderRight' : 'borderLeft']: '4px solid #8b5cf6' }}>
          <div style={{ padding: '1rem', background: '#8b5cf620', borderRadius: '12px' }}>
            <Scale size={32} color="#8b5cf6" />
          </div>
          <div>
            <div style={{ fontSize: '0.9rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>{_t('الوعاء الزكوي التقريبي', 'Approximate Zakat Base')}</div>
        ...
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '1.5rem', background: '#14b8a6', color: 'white' }}>
          ...
          <div>
            <div style={{ fontSize: '0.9rem', opacity: 0.9, marginBottom: '0.25rem' }}>{_t('الزكاة التقديرية (2.5%)', 'Estimated Zakat (2.5%)')}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem' }}>
        <div className="card">
          <h2 style={{ fontSize: '1.25rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            {_t('التعديلات على صافي الربح والوعاء الزكوي', 'Adjustments to Net Profit & Zakat Base')}
          </h2>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: lang === 'ar' ? 'right' : 'left' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid var(--border)', color: 'var(--text-muted)' }}>
                  <th style={{ padding: '1rem' }}>{_t('البيان', 'Description')}</th>
                  <th style={{ padding: '1rem', textAlign: 'center' }}>{_t('النوع', 'Type')}</th>
                  <th style={{ padding: '1rem' }}>{_t('المبلغ', 'Amount')}</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={idx} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '1rem', textAlign: 'center' }}>
                      <span style={{ 
                        ...
                        {adj.type === 'add' ? _t('إضافة', 'Addition') : _t('خصم', 'Deduction')}
                      </span>
                    </td>
        ...
        <div className="card">
          <h2 style={{ fontSize: '1.25rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            {_t('الجداول المرفقة', 'Attached Schedules')}
          </h2>
            ...
            {data?.schedules.map((sch: any, idx: number) => (
              <li key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'var(--bg-secondary)', borderRadius: '8px' }}>
                ...
                <span style={{ 
                  ...
                  {sch.status === 'ready' ? _t('مكتمل', 'Complete') : _t('غير مكتمل', 'Incomplete')}
                </span>
              </li>
            ))}
          </ul>
          ...
          <div style={{ marginTop: '2rem' }}>
            <button disabled className="btn btn-primary" style={{ width: '100%', opacity: 0.5, cursor: 'not-allowed' }}>
              {_t('اعتماد وإغلاق الإقرار الزكوي', 'Approve & Close Zakat Declaration')}
            </button>
            <div style={{ textAlign: 'center', fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.75rem' }}>
              {_t('يتطلب اكتمال جميع الجداول المرفقة', 'Requires all attached schedules to be completed')}
            </div>
          </div>
        </div>
      </div>
