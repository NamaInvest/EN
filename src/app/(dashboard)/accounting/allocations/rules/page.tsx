    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold tracking-tight">{_t('مخصصات مراكز التكلفة', 'Cost Center Allocations')}</h1>
                <div className="space-x-2">
                    <Button>{_t('إنشاء Allocation Rule', 'Create Allocation Rule')}</Button>
                    <Button variant="outline">{_t('تشغيل المخصصات الآن', 'Run Allocations Now')}</Button>
                </div>
            </div>
            
            <Card>
                <CardHeader>
                    <CardTitle>{_t('قواعد التخصيص النشطة', 'Active Allocation Rules')}</CardTitle>
                </CardHeader>
                <CardContent>
