    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6 p-6">
            <h1 className="text-3xl font-bold flex items-center gap-2">
                <Lock className="w-8 h-8 text-blue-600" />{_t('Period Close Engine', 'Period Close Engine')}</h1>
            <p className="text-gray-500">{_t('Run monthly depreciation, FX revaluation, and lock accounting periods.', 'Run monthly depreciation, FX revaluation, and lock accounting periods.')}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="p-6 space-y-4 border-t-4 border-t-orange-400">
                    ...
                    <Button onClick={() => handleAction('fx_reval', { exchangeRate: 3.75, currencyId: 2 })} disabled={loading} variant="outline" className="w-full">{_t('Run FX Revaluation', 'Run FX Revaluation')}</Button>
                </Card>
                ...
                <Card className="p-6 space-y-4 border-t-4 border-t-indigo-400">
                    ...
                    <p className="text-sm text-gray-500">{_t('Calculates straight-line depreciation for all active fixed assets for this month.', 'Calculates straight-line depreciation for all active fixed assets for this month.')}</p>
                    <Button onClick={() => handleAction('depreciation')} disabled={loading} variant="outline" className="w-full">{_t('Run Monthly Depreciation', 'Run Monthly Depreciation')}</Button>
                </Card>
                ...
                <Card className="p-6 space-y-4 md:col-span-2 border-t-4 border-t-red-500 bg-red-50">
                    ...
                    <p className="text-sm text-red-600">{_t('Closes the selected month. No further journal entries can be posted or modified in this period.', 'Closes the selected month. No further journal entries can be posted or modified in this period.')}</p>
