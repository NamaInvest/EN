    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold tracking-tight">{_t('الوصفات الطبية الإلكترونية (e-Rx)', 'Electronic Prescriptions (e-Rx)')}</h1>
                <Button>{_t('إنشاء New Prescription', 'Create New Prescription')}</Button>
            </div>
            
            <Card>
                <CardHeader>
                    <CardTitle>{_t('الوصفات الطبية الأخيرة', 'Recent Prescriptions')}</CardTitle>
                </CardHeader>
                <CardContent>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                    <tr>
                                        <th className="px-4 py-3">{_t('معرف البريد الإلكتروني', 'e-Rx ID')}</th>
                                        <th className="px-4 py-3">{_t('التاريخ', 'Date')}</th>
                                        <th className="px-4 py-3">{_t('مريض', 'Patient')}</th>
                                        <th className="px-4 py-3">{_t('الطبيب الموصوف', 'Prescribing Doctor')}</th>
                                        <th className="px-4 py-3">{_t('الأدوية', 'Medications')}</th>
                                        ...
                                        <tr key={idx} className="border-b dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800">
                                            ...
                                            <td className="px-4 py-3">
                                                {rx.status === 'ACTIVE' ? (
                                                    ...
                                                    <Badge variant="outline">{_t('الاستغناء عنها', 'Dispensed')}</Badge>
                                                )}
                                            </td>
