...
import { Factory, TrendingUp, Activity, BarChart, Settings, Download } from 'lucide-react';
import { Card } from '@/components/ui/card';
import { Button } from '@/components/ui/button';

/**
 * AdvancedManufacturingDashboard - Advanced Manufacturing and MRP Module Dashboard
 * fully localized with bilingual helper _t supporting both Arabic and English interfaces.
 */
export default function AdvancedManufacturingDashboard() {
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-slate-800">
                          {_t('التصنيع المتقدم V3', 'Advanced Manufacturing V3')}
                        </h1>
                        <p className="text-slate-500 mt-1">
                          {_t('قوائم مواد متعددة المستويات، MRP، توجيه أرضية الورشة، رقابة الجودة', 'Multi-Level BOMs, MRP Runs, Shop Floor Routing, Quality Control')}
                        </p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <Card className="p-5 border-l-4 border-l-indigo-500 hover:shadow-md transition-shadow">
                    <p className="text-sm font-medium text-slate-500">
                      {_t('الكفاءة الإجمالية للمعدات (OEE)', 'Overall Equipment Effectiveness (OEE)')}
                    </p>
                ...
                <Card className="p-5 border-l-4 border-l-emerald-500 hover:shadow-md transition-shadow">
                    <p className="text-sm font-medium text-slate-500">
                      {_t('انحراف العائد', 'Yield Variance')}
                    </p>
                    <h3 className="text-2xl font-bold mt-2 text-slate-800">
                      15%
                    </h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <Card className="col-span-2 p-6 h-96 flex flex-col items-center justify-center border-dashed bg-slate-50">
                    ...
                    <p className="text-sm text-slate-500 text-center max-w-sm mt-2">
                        {_t('ستُعرض المخططات التشغيلية هنا اعتماداً على V3 EventBus حسب احتياجات قطاع التصنيع المتقدم.', 'Operational charts will be rendered here via V3 EventBus telemetry according to Advanced Manufacturing sector requirements.')}
                    </p>
                </Card>
