        ...
        <div className="max-w-7xl mx-auto space-y-6 p-6">
            <div className="flex justify-between items-center bg-white p-6 rounded-xl border shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-slate-800">{_t('الخدمات المهنية V3', 'Professional Services V3')}</h1>
                        <p className="text-slate-500 mt-1">{_t('ساعات قابلة للفوترة، وإدارة التجنيب، وصرف النفقات.', 'Billable Hours, Retainer Management, Expense disbursement.')}</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <Card className="p-5 border-l-4 border-l-indigo-500 hover:shadow-md transition-shadow">
                    <p className="text-sm font-medium text-slate-500">{_t('معدل الاستخدام', 'Utilization Rate')}</p>
                ...
                <Card className="p-5 border-l-4 border-l-emerald-500 hover:shadow-md transition-shadow">
                    <p className="text-sm font-medium text-slate-500">{_t('معدل التنفيذ', 'Realization Rate')}</p>
                ...
                <Card className="p-5 border-l-4 border-l-indigo-500 hover:shadow-md transition-shadow">
                    <p className="text-sm font-medium text-slate-500">{_t('الإيرادات لكل موظف', 'Revenue per Employee')}</p>
                    <h3 className="text-2xl font-bold mt-2 text-slate-800">
                        1,204
                    </h3>
                    <div className="mt-2 flex items-center text-xs text-emerald-600 font-medium">
                ...
                <Card className="p-5 border-l-4 border-l-emerald-500 hover:shadow-md transition-shadow">
                    <p className="text-sm font-medium text-slate-500">{_t('تحويل الأعمال قيد التنفيذ إلى نقد', 'WIP-to-Cash')}</p>
                    <h3 className="text-2xl font-bold mt-2 text-slate-800">
                        15%
                    </h3>
                    <div className="mt-2 flex items-center text-xs text-emerald-600 font-medium">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <Card className="col-span-2 p-6 h-96 flex flex-col items-center justify-center border-dashed bg-slate-50">
                    ...
                    <p className="text-sm text-slate-500 text-center max-w-sm mt-2">{_t('سيعرض القياس المباشر عن بعد من V3 EventBus مخططات تشغيلية هنا بناءً على الاحتياجات المحددة للخدمات الاحترافية.', 'Live telemetry from the V3 EventBus will render operational charts here based on the specific needs of Professional Services.')}</p>
                </Card>
