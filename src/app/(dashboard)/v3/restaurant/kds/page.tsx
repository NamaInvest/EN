    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white p-4">
            <header className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-yellow-500">{_t('نظام عرض المطبخ (KDS)', 'Kitchen Display System (KDS)')}</h1>
                <div className="flex space-x-4">
                    <div className="text-center"><div className="text-2xl font-bold text-red-500">2</div><div className="text-xs uppercase">{_t('قيد الانتظار', 'Pending')}</div></div>
                    ...
                    <Card key={idx} className="min-w-[300px] bg-gray-800 border-gray-700 text-white shrink-0">
                        ...
                        <CardContent className="pt-4 space-y-4">
                            ...
                            <div className="pt-4 space-y-2">
                                {t.status === 'NEW' && <Button className="w-full bg-blue-600 hover:bg-blue-700">{_t('ابدأ بالتحضير', 'Start Preparing')}</Button>}
                                {t.status === 'PREPARING' && <Button className="w-full bg-green-600 hover:bg-green-700">{_t('مارك جاهز', 'Mark Ready')}</Button>}
                            </div>
                        </CardContent>
                    </Card>
                ))}
