        ...
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                ...
                <div className="space-x-2">
                    <Button variant="outline">{_t('قائمة التحفظات', 'Reservations List')}</Button>
                    <Button>{_t('حجز جديد', 'New Reservation')}</Button>
                </div>
            </div>
                ...
                {tables.map((t, idx) => (
                    <Card key={idx} className={`border-2 ${getStatusColor(t.status)}`}>
                        ...
                        <CardContent className="text-center space-y-3">
                            ...
                            <div className="pt-2">
                                {t.status === 'AVAILABLE' && <Button size="sm" className="w-full">{_t('دخول', 'Walk-in')}</Button>}
                                {t.status === 'RESERVED' && <Button size="sm" className="w-full">{_t('ضيف المقعد', 'Seat Guest')}</Button>}
                                {t.status === 'DIRTY' && <Button size="sm" variant="outline" className="w-full">{_t('مارك كلين', 'Mark Clean')}</Button>}
                                {t.status === 'OCCUPIED' && <Button size="sm" variant="outline" className="w-full">{_t('عرض الطلب', 'View Order')}</Button>}
                            </div>
                        </CardContent>
                    </Card>
                ))}
