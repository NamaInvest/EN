...
export default function RealEstateLeases() {
  ...
  return (
    <div className="p-6 bg-slate-100 min-h-screen text-slate-800 space-y-6 max-w-7xl mx-auto">      {/* Global System Features Bar */} \n      <div className="bg-slate-900 border border-slate-700 p-4 rounded-xl shadow-xl flex justify-between items-center mb-6 animate-fade-in">\n        <div className="flex items-center gap-2">\n          <span className="text-yellow-400 font-black tracking-widest text-sm border border-yellow-400/50 bg-yellow-400/10 px-2 py-1 rounded">{_t('ميزات المؤسسة العالمية', 'GLOBAL ENTERPRISE FEATURES')}</span>\n        </div>\n        <div className="flex gap-3">\n        <Button className="bg-teal-600 text-white font-bold hover:opacity-90 shadow-lg"><KeyRound className="w-4 h-4 mr-2"/>{_t('إنشاء رقم التعريف الشخصي للقفل الذكي', 'Generate Smart Lock PIN')}</Button>\n        <Button className="bg-indigo-500 text-white font-bold hover:opacity-90 shadow-lg"><Glasses className="w-4 h-4 mr-2"/> 3D Virtual Tour</Button>\n        <Button className="bg-sky-500 text-white font-bold hover:opacity-90 shadow-lg"><MessageSquare className="w-4 h-4 mr-2"/>{_t('البوابة الاجتماعية للمستأجر', 'Tenant Social Portal')}</Button>\n        </div>\n      </div>
      <div className="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <div>
          <h1 className="text-3xl font-black flex items-center gap-3 text-slate-800"><Building className="text-teal-600 w-8 h-8"/>{_t('برج الفيصلية', 'Al-Faisaliah Tower')}</h1>
          <p className="text-slate-500 mt-1 font-medium">Commercial Property • 45 Units • 92% Occupancy</p>
        </div>
        <Button onClick={() => { fetch("/api/v3/realestate/leases", { method: "POST", body: JSON.stringify({ propertyId: 1, tenantId: 1, rentAmount: 120000 }) }).then(()=>toastSuccess("Lease Contract Created in DB!")); }} className="bg-teal-600 hover:bg-teal-700 font-bold"><Key className="w-4 h-4 mr-2"/>{_t('عقد إيجار جديد', 'New Lease Contract')}</Button>
      </div>

      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-2 space-y-6">
          <Card className="p-6 bg-white shadow-sm border-slate-200">
            <h2 className="text-xl font-bold flex items-center gap-2 mb-4 border-b pb-4"><Wallet className="text-teal-600"/>{_t('PDC Vault (الشيكات المؤجلة)', 'PDC Vault (Post-Dated Checks)')}</h2>
        ...
        <div className="space-y-6">
          <Card className="p-6 shadow-sm border-slate-200 bg-white">
            <h2 className="font-bold flex items-center gap-2 mb-4"><Wrench className="w-5 h-5 text-teal-600"/>{_t('تذاكر الصيانة', 'Maintenance Tickets')}</h2>
            <div className="space-y-3">
              <div className="p-3 border-l-4 border-l-red-500 rounded-r-lg bg-slate-50 shadow-sm">
                <p className="font-bold text-sm">{_t('تسرب التيار المتردد', 'AC Leakage')}</p>
