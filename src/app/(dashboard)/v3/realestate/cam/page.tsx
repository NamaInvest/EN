    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold tracking-tight">{_t('CAM Reconciliation', 'CAM Reconciliation')}</h1>
                <Button>{_t('Run Annual CAM Recon', 'Run Annual CAM Recon')}</Button>
            </div>
            
            <Card>
                <CardHeader>
                    <CardTitle>{_t('Common Area Maintenance (CAM) by Property', 'Common Area Maintenance (CAM) by Property')}</CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    <th className="px-4 py-3">{_t('Property ID', 'Property ID')}</th>
                                    <th className="px-4 py-3">{_t('الاسم', 'Name')}</th>
                                    <th className="px-4 py-3">{_t('Total Area (sqm)', 'Total Area (sqm)')}</th>
                                    <th className="px-4 py-3">{_t('الفترة (Period)', 'Period')}</th>
                                    <th className="px-4 py-3 text-right">{_t('Total CAM Expenses (SAR)', 'Total CAM Expenses (SAR)')}</th>
                                    ...
                                    <tr key={idx} className="border-b dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800">
                                        ...
                                        <td className="px-4 py-3">
                                            {prop.status === 'RECONCILED' ? (
                                                <Badge className="bg-green-100 text-green-800">{_t('Reconciled', 'Reconciled')}</Badge>
                                            ) : (
                                                <Badge variant="outline" className="bg-yellow-50 text-yellow-800">{_t('Pending', 'Pending')}</Badge>
                                            )}
                                        </td>
                                        <td className="px-4 py-3 text-right space-x-2">
                                            <Button size="sm" variant="outline">{_t('View Breakdown', 'View Breakdown')}</Button>
                                            {prop.status === 'PENDING_RECONCILIATION' && (
                                                <Button size="sm" variant="outline" className="text-blue-600 border-blue-500">{_t('Reconcile', 'Reconcile')}</Button>
                                            )}
                                        </td>
                                    </tr>
                                ))}
