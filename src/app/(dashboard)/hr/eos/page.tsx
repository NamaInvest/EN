...
const STATUS_BADGES: Record<string, { color: string; arLabel: string; enLabel: string }> = {
    DRAFT: { color: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300', arLabel: 'مسودة', enLabel: 'Draft' },
        ...
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 text-sm">{_t('وفقاً للمواد 84-88 من نظام العمل السعودي', 'Per Articles 84-88 of Saudi Labor Law')}</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white dark:bg-gray-800 p-5 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                    <p className="text-sm font-medium text-gray-500 mb-1">{_t('مسودات', 'Drafts')}</p>
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white font-mono">{draftCount}</h3>
                </div>
            ...
            {/* Calculator */}
            {showCalc && (
                <div className="bg-white dark:bg-gray-800 border border-purple-200 dark:border-purple-800 rounded-lg p-6 shadow-sm">
                    <h2 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">{_t('حاسبة مكافأة نهاية الخدمة', 'EOS Calculator')}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{_t('تاريخ الانتهاء', 'End Date')}</label>
                            <input type="date" value={form.endDate} onChange={e => setForm({...form, endDate: e.target.value})}
                                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{_t('سبب الإنهاء', 'Reason')}</label>
                    ...
                    {calcResult && (
                        <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700">
                            <h3 className="text-md font-semibold text-gray-800 dark:text-gray-200 mb-3">{_t('نتيجة الحساب', 'Calculation Result')}</h3>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                                <div><span className="text-gray-500">{_t('سنوات الخدمة', 'Years of Service')}:</span> <span className="font-bold">{Number(calcResult.yearsOfService).toFixed(2)}</span></div>
                                <div><span className="text-gray-500">{_t('أول 5 سنوات', 'First 5 Years')}:</span> <span className="font-bold font-mono">{Number(calcResult.firstFiveYearsAmount).toLocaleString()}</span></div>
                                <div><span className="text-gray-500">{_t('بعد 5 سنوات', 'After 5 Years')}:</span> <span className="font-bold font-mono">{Number(calcResult.remainingYearsAmount).toLocaleString()}</span></div>
                                <div><span className="text-gray-500">{_t('معامل الاستقالة', 'Factor')}:</span> <span className="font-bold">{(Number(calcResult.resignationFactor) * 100).toFixed(0)}%</span></div>
                            </div>
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                    <h2 className="text-lg font-medium text-gray-900 dark:text-white">{_t('سجل التسويات', 'Settlement Records')}</h2>
                </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead>
                            <tr>
                                ...
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('السبب', 'Reason')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('سنوات الخدمة', 'Years')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('المكافأة', 'EOS Amount')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('الصافي', 'Net')}</th>
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {loading ? (
                                ...
                                <tr><td colSpan={7} className="text-center py-10 text-gray-500">{_t('لا توجد تسويات', 'No settlements')}</td></tr>
                                ...
                                return (
                                    <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{c.employee?.name || c.employee?.fullName || `#${c.employeeId}`}</td>
                                        <td className="px-6 py-4 text-sm text-center text-gray-500">{reasonLabel ? _t(reasonLabel.ar, reasonLabel.en) : c.reasonForLeaving}</td>
