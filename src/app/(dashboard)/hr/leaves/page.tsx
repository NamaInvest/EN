        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 text-sm">{_t('وفقاً لنظام العمل السعودي — المواد 109-116', 'Per Saudi Labor Law — Articles 109-116')}</p>
                </div>
                <div className="flex gap-2">
                    <button onClick={runAccrual} disabled={accrualRunning}
                        className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center disabled:opacity-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white dark:bg-gray-800 p-5 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                    <p className="text-sm font-medium text-gray-500 mb-1">{_t('إجمالي الطلبات', 'Total Requests')}</p>
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white font-mono">{requests.length}</h3>
                </div>
            ...
            {/* New Request Form */}
            {showForm && (
                <div className="bg-white dark:bg-gray-800 border border-blue-200 dark:border-blue-800 rounded-lg p-6 shadow-sm">
                    <h2 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">{_t('طلب إجازة جديد', 'New Leave Request')}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{_t('رقم الموظف', 'Employee ID')}</label>
                            <input type="number" value={form.employeeId} onChange={e => setForm({...form, employeeId: e.target.value})}
                                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{_t('نوع الإجازة', 'Leave Type')}</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{_t('من تاريخ', 'Start Date')}</label>
                            <input type="date" value={form.startDate} onChange={e => setForm({...form, startDate: e.target.value})}
                                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{_t('إلى تاريخ', 'End Date')}</label>
                            <input type="date" value={form.endDate} onChange={e => setForm({...form, endDate: e.target.value})}
                                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white" />
                        </div>
                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{_t('السبب', 'Reason')}</label>
                    ...
                    <div className="flex gap-2 mt-4">
                        ...
                        <button onClick={() => setShowForm(false)}
                            className="px-6 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 text-sm">
                            {_t('إلغاء', 'Cancel')}
                        </button>
                    </div>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50">
                    <h2 className="text-lg font-medium text-gray-900 dark:text-white">{_t('طلبات الإجازات', 'Leave Requests')}</h2>
                    <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                        className="px-3 py-1 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-800 dark:text-white">
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-white dark:bg-gray-800">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{_t('الموظف', 'Employee')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('النوع', 'Type')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('من', 'From')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('إلى', 'To')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('الأيام', 'Days')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('الحالة', 'Status')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('إجراءات', 'Actions')}</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {loading ? (
                                ...
                                <tr><td colSpan={7} className="text-center py-10 text-gray-500">{_t('لا توجد طلبات', 'No requests found')}</td></tr>
                            ) : requests.map((r: any) => {
                                ...
                                return (
                                    <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{r.employee?.name || r.employee?.fullName || `#${r.employeeId}`}</td>
                                        <td className="px-6 py-4 text-sm text-center text-gray-500">{leaveLabel ? _t(leaveLabel.ar, leaveLabel.en) : r.leaveType}</td>
                                        <td className="px-6 py-4 text-sm text-center text-gray-500 font-mono">{new Date(r.startDate).toLocaleDateString()}</td>
                                        <td className="px-6 py-4 text-sm text-center text-gray-500 font-mono">{new Date(r.endDate).toLocaleDateString()}</td>
                                        <td className="px-6 py-4 text-sm text-center font-bold text-gray-700 dark:text-gray-300">{r.days}</td>
