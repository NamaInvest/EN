        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 text-sm">{_t('تتبع انتهاء الإقامات والرخص والوثائق الرسمية', 'Track expiry of Iqamas, licenses, and official documents')}</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div className="bg-white dark:bg-gray-800 p-5 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                    <p className="text-sm font-medium text-gray-500 mb-1">{_t('إجمالي التنبيهات', 'Total Alerts')}</p>
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white font-mono">{summary.totalDocuments}</h3>
                </div>
                ...
                <div className={`p-5 rounded-lg border shadow-sm ${summary.complianceRate >= 90 ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800'}`}>
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">{_t('نسبة الامتثال', 'Compliance Rate')}</p>
                ...
                <div className="flex space-x-4 rtl:space-x-reverse">
                    {tabs.map(tab => (
                        <button key={tab.key}
                            ...
                            className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.key ? `border-blue-600 ${tab.color}` : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-900/50">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{_t('الحامل', 'Holder')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('نوع الوثيقة', 'Document Type')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('الرقم', 'Number')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('تاريخ الانتهاء', 'Expiry Date')}</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">{_t('الأيام المتبقية', 'Days Left')}</th>
                                ...
                                return (
                                    <tr key={alert.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">
                                            ...
                                            <span className="text-xs text-gray-400">{alert.holderType === 'EMPLOYEE' ? _t('موظف', 'Employee') : _t('شركة', 'Company')}</span>
                                        </td>
                                        <td className="px-6 py-4 text-sm text-center text-gray-600 dark:text-gray-300">{_t(docLabel.ar, docLabel.en)}</td>
                                        <td className="px-6 py-4 text-sm text-center text-gray-500 font-mono">{alert.documentNumber}</td>
                                        <td className="px-6 py-4 text-sm text-center text-gray-500 font-mono">{new Date(alert.expiryDate).toLocaleDateString()}</td>
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <div className="flex gap-1 justify-center">
                                                ...
                                                <button onClick={() => handleDismiss(alert.id)}
                                                    className="px-3 py-1 border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 rounded text-xs hover:bg-gray-50 dark:hover:bg-gray-700">
                                                    {_t('تجاهل', 'Dismiss')}
                                                </button>
                                            </div>
                                        </td>
            ...
            {renewModal.show && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
                    <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md shadow-xl">
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{_t('تجديد الوثيقة', 'Renew Document')}</h3>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{_t('تاريخ الانتهاء الجديد', 'New Expiry Date')}</label>
                        ...
                        <div className="flex gap-2 mt-4">
                            ...
                            <button onClick={() => setRenewModal({ show: false, alertId: null, newDate: '' })}
                                className="px-6 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-gray-700 dark:text-gray-300 text-sm">
                                {_t('إلغاء', 'Cancel')}
                            </button>
                        </div>
                    </div>
