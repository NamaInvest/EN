...
const formSchema = z.object({
    ...
    monthlyDeduction: z.number().min(1, 'Monthly deduction must be at least 1'),
                  ...
                  <div className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-lg shadow-2xl p-8 transform animate-in zoom-in-95">
                      ...
                      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                          <div>
                              ...
                              <select 
                                  className={`w-full bg-slate-800 border ${errors.employeeId ? 'border-red-500' : 'border-slate-700'} rounded-xl px-4 py-3 text-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all appearance-none`}
                                  {...register('employeeId')}
                              >
                          ...
                          <div className="grid grid-cols-2 gap-4">
                              <div>
                                  ...
                                  <input 
                                      type="number" min="1" 
                                      className={`w-full bg-slate-800 border ${errors.amount ? 'border-red-500' : 'border-slate-700'} rounded-xl px-4 py-3 text-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all font-mono`}
                              ...
                              <div>
                                  ...
                                  <input 
                                      type="number" min="1" 
                                      className={`w-full bg-slate-800 border ${errors.monthlyDeduction ? 'border-red-500' : 'border-slate-700'} rounded-xl px-4 py-3 text-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all font-mono text-rose-400`}
                                      {...register('monthlyDeduction', { valueAsNumber: true })}
                                  />
                                  {errors.monthlyDeduction && <span className="text-red-500 text-xs mt-1 block">{errors.monthlyDeduction.message}</span>}
                              </div>
                          </div>
                          <div>
                              ...
                              <input 
                                  type="text" 
                                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all"
                                  {...register('reason')}
                              />
                          </div>
                          <div className="flex space-x-4 space-x-reverse pt-4">
                              <button type="submit" className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-xl font-medium transition-all shadow-lg shadow-indigo-500/30" disabled={saving}>
                                  {saving ? 'جاري الحفظ...' : 'اعتماد السلفة'}
                              </button>
                              <button type="button" onClick={() => { setShowModal(false); reset(); }} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 py-3 rounded-xl font-medium transition-all">إلغاء</button>
                          </div>
                      </form>
                  </div>
              </div>
