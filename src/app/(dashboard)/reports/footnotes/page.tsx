    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold tracking-tight">{_t('حواشي البيان المالي', 'Financial Statement Footnotes')}</h1>
                <div className="space-x-2">
                    <Button variant="outline">{_t('التعبئة التلقائية', 'Auto-Populate')}</Button>
                    <Button>{_t('حاشية سفلية جديدة', 'New Footnote')}</Button>
                </div>
            </div>
            
            <Card>
                <CardHeader>
                    <CardTitle>{_t('الإفصاحات والحواشي (السنة المالية 2026)', 'Disclosures & Footnotes (FY 2026)')}</CardTitle>
                </CardHeader>
                <CardContent>
                        ...
                        {footnotes.map((fn, idx) => (
                            <div key={idx} className="border p-4 rounded-lg bg-gray-50 dark:bg-gray-800">
                                ...
                                <div className="mt-4 flex space-x-2">
                                    <Button size="sm" variant="outline">{_t('تحرير المحتوى', 'Edit Content')}</Button>
