    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold tracking-tight">{_t('التقارير القطاعية (المعيار الدولي لإعداد التقارير المالية 8)', 'Segment Reporting (IFRS 8)')}</h1>
                <Button>{_t('تقرير قطاع التصدير', 'Export Segment Report')}</Button>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <Card className="bg-blue-50 border-blue-200">
                    <CardHeader className="pb-2"><CardTitle className="text-blue-800 text-sm">{_t('إجمالي إيرادات القطاع', 'Total Segment Revenue')}</CardTitle></CardHeader>
                    <CardContent><div className="text-3xl font-bold text-blue-900">{_t('7,700,000 ريال سعودي', 'SAR 7,700,000')}</div></CardContent>
                </Card>
                <Card className="bg-green-50 border-green-200">
                    <CardHeader className="pb-2"><CardTitle className="text-green-800 text-sm">{_t('إجمالي هامش القطاع', 'Total Segment Margin')}</CardTitle></CardHeader>
                    <CardContent><div className="text-3xl font-bold text-green-900">{_t('3,700,000 ريال سعودي', 'SAR 3,700,000')}</div></CardContent>
                </Card>
            </div>

            <Card>
                <CardHeader>
                    <CardTitle>{_t('الأداء حسب قطاع التشغيل', 'Performance by Operating Segment')}</CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    <th className="px-4 py-3">{_t('اسم الجزء', 'Segment Name')}</th>
                                ...
                                {segments.map((seg, idx) => (
                                    <tr key={idx} className="border-b dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800">
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            <Button size="sm" variant="outline">{_t('التنقل', 'Drilldown')}</Button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
