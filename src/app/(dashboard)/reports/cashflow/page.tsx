...
export default function CashFlowDashboard() {
 ...
 if (isLoading) {
 return <div className="p-8 min-h-screen bg-[#F9FAFB] flex items-center justify-center text-slate-500">Calculating Cash Flow Forecast...</div>;
 }

 if (!data) {
 return <div className="p-8 min-h-screen bg-[#F9FAFB] flex items-center justify-center text-red-500">Failed to load forecast data.</div>;
 }
 ...
 <div>
 <h1 className="text-3xl font-bold text-slate-800">Cash Flow Forecast</h1>
 ...
 <p className="text-sm font-medium text-slate-500">Opening Balance (Today)</p>
 <p className="text-2xl font-bold text-slate-800 mt-2">${data.openingBalance.toLocaleString()}</p>
 </div>
 <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
 <p className="text-sm font-medium text-slate-500">Expected 30-Day Inflows</p>
 <p className="text-2xl font-bold text-green-600 mt-2">
 +${data.forecast.reduce((acc: number, day: any) => acc + day.inflow, 0).toLocaleString()}
 </p>
 </div>
 <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
 <p className="text-sm font-medium text-slate-500">Expected 30-Day Outflows</p>
 ...
 <h3 className="text-lg font-bold text-slate-800 mb-6">Daily Forecast Matrix</h3>
 ...
 <th className="py-3 px-4 font-medium text-slate-500 text-sm rounded-tr-lg">Key Drivers</th>
 </tr>
 </thead>
 <tbody>
