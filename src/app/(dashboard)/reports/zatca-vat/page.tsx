...
export default function ZatcaVatReturn() {
 ...
 <p className="text-slate-500 mt-1 text-sm">{_t('نموذج الإقرار الضريبي الرسمي للمملكة العربية السعودية', 'Official VAT Return Form for the Kingdom of Saudi Arabia')}</p>
 </div>
 <div className="flex gap-2">
 <select 
 value={period}
 onChange={(e) => setPeriod(e.target.value)}
 className="px-4 py-2 border border-slate-300 rounded-md shadow-sm text-sm font-medium text-slate-700 bg-white hover:bg-slate-50 focus:ring-green-500 focus:border-green-500 "
 >
 ...
 <div className="flex-1 overflow-y-auto min-h-0 bg-white border border-slate-200 rounded-lg shadow-sm">
 
 {/* 1. Value on Sales */}
 <div className="p-6">
 <h2 className="text-xl font-bold text-slate-900 mb-4 border-b pb-2 ">{_t('1. ضريبة القيمة المضافة على المبيعات (ضريبة المخرجات)', '1. VAT on Sales (Output Tax)')}</h2>
 <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 mb-6 border border-slate-200 rounded-lg">
 <thead className="bg-slate-100 ">
 <tr>
 <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase">{_t('البند', 'Item')}</th>
 <th className="px-4 py-3 text-right text-xs font-semibold text-slate-600 uppercase">{_t('المبلغ (ريال)', 'Amount (SAR)')}</th>
 <th className="px-4 py-3 text-right text-xs font-semibold text-slate-600 uppercase">{_t('التعديل (ريال)', 'Adjustment (SAR)')}</th>
 <th className="px-4 py-3 text-right text-xs font-semibold text-slate-600 uppercase">{_t('مبلغ الضريبة (ريال)', 'VAT Amount (SAR)')}</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
 <tr className="bg-white ">
 <td className="px-4 py-3 text-sm text-slate-900 font-medium">{_t('1. المبيعات الخاضعة للنسبة الأساسية (15%)', '1. Standard rated sales (15%)')}</td>
 ...
 <td className="px-4 py-3 text-sm text-right font-mono font-medium text-slate-900 ">{formatCurrency(data.sales.standard.vat)}</td>
 </tr>
 <tr className="bg-slate-50 ">
 <td className="px-4 py-3 text-sm text-slate-900 font-medium">{_t('2. المبيعات للعملاء في دول مجلس التعاون', '2. Sales to customers in GCC countries')}</td>
 <td className="px-4 py-3 text-sm text-right font-mono">0.00</td>
 <td className="px-4 py-3 text-sm text-right font-mono">0.00</td>
 <td className="px-4 py-3 text-sm text-right font-mono bg-slate-200 text-slate-500">N/A</td>
 </tr>
 <tr className="bg-white ">
 <td className="px-4 py-3 text-sm text-slate-900 font-medium">{_t('3. المبيعات المحلية الخاضعة للنسبة الصفرية', '3. Zero-rated domestic sales')}</td>
 <td className="px-4 py-3 text-sm text-right font-mono">{formatCurrency(data.sales.zeroRated.amount)}</td>
 <td className="px-4 py-3 text-sm text-right font-mono">0.00</td>
 <td className="px-4 py-3 text-sm text-right font-mono bg-slate-200 text-slate-500">N/A</td>
 </tr>
 <tr className="bg-slate-50 ">
 <td className="px-4 py-3 text-sm text-slate-900 font-medium">{_t('4. الصادرات', '4. Exports')}</td>
 <td className="px-4 py-3 text-sm text-right font-mono">{formatCurrency(data.sales.exports.amount)}</td>
 <td className="px-4 py-3 text-sm text-right font-mono">0.00</td>
 <td className="px-4 py-3 text-sm text-right font-mono bg-slate-200 text-slate-500">N/A</td>
 </tr>
 <tr className="bg-white ">
 <td className="px-4 py-3 text-sm text-slate-900 font-medium">{_t('5. المبيعات المعفاة', '5. Exempt sales')}</td>
 <td className="px-4 py-3 text-sm text-right font-mono">{formatCurrency(data.sales.exempt.amount)}</td>
 <td className="px-4 py-3 text-sm text-right font-mono">0.00</td>
 <td className="px-4 py-3 text-sm text-right font-mono bg-slate-200 text-slate-500">N/A</td>
 </tr>
 <tr className="bg-slate-100 font-bold">
 <td className="px-4 py-3 text-sm text-slate-900 ">{_t('6. إجمالي المبيعات', '6. Total Sales')}</td>
 ...
 <h2 className="text-xl font-bold text-slate-900 mb-4 border-b pb-2 ">{_t('2. ضريبة القيمة المضافة على المشتريات (ضريبة المدخلات)', '2. VAT on Purchases (Input Tax)')}</h2>
 <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 mb-6 border border-slate-200 rounded-lg">
 <thead className="bg-slate-100 ">
 <tr>
 <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase">{_t('البند', 'Item')}</th>
 ...
 <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
 <tr className="bg-white ">
 <td className="px-4 py-3 text-sm text-slate-900 font-medium">{_t('7. المشتريات المحلية الخاضعة للنسبة الأساسية (15%)', '7. Standard rated domestic purchases (15%)')}</td>
 ...
 <td className="px-4 py-3 text-sm text-right font-mono font-medium text-slate-900 ">{formatCurrency(data.purchases.standard.vat)}</td>
 </tr>
 <tr className="bg-slate-50 ">
 <td className="px-4 py-3 text-sm text-slate-900 font-medium">{_t('8. الاستيرادات الخاضعة لضريبة القيمة المضافة المدفوعة في الجمارك', '8. Imports subject to VAT paid at customs')}</td>
 ...
 <td className="px-4 py-3 text-sm text-right font-mono font-medium text-slate-900 ">{formatCurrency(data.purchases.importsPaidAtCustoms.vat)}</td>
 </tr>
 <tr className="bg-white ">
 <td className="px-4 py-3 text-sm text-slate-900 font-medium">{_t('9. الاستيرادات الخاضعة للضريبة والتي تطبق عليها آلية الاحتساب العكسي', '9. Imports subject to VAT accounted for through reverse charge mechanism')}</td>
 ...
 <td className="px-4 py-3 text-sm text-right font-mono font-medium text-slate-900 ">{formatCurrency(data.purchases.importsReverseCharge.vat)}</td>
 </tr>
 <tr className="bg-slate-50 ">
 <td className="px-4 py-3 text-sm text-slate-900 font-medium">{_t('10. المشتريات الخاضعة للنسبة الصفرية', '10. Zero-rated purchases')}</td>
 <td className="px-4 py-3 text-sm text-right font-mono">{formatCurrency(data.purchases.zeroRated.amount)}</td>
 <td className="px-4 py-3 text-sm text-right font-mono">0.00</td>
 <td className="px-4 py-3 text-sm text-right font-mono bg-slate-200 text-slate-500">N/A</td>
 </tr>
 <tr className="bg-white ">
 <td className="px-4 py-3 text-sm text-slate-900 font-medium">{_t('11. المشتريات المعفاة', '11. Exempt purchases')}</td>
 <td className="px-4 py-3 text-sm text-right font-mono">{formatCurrency(data.purchases.exempt.amount)}</td>
 <td className="px-4 py-3 text-sm text-right font-mono">0.00</td>
 <td className="px-4 py-3 text-sm text-right font-mono bg-slate-200 text-slate-500">N/A</td>
 </tr>
 <tr className="bg-slate-100 font-bold">
 <td className="px-4 py-3 text-sm text-slate-900 ">{_t('12. إجمالي المشتريات', '12. Total Purchases')}</td>
 ...
 <p className="text-sm text-slate-500 mt-1">{_t('إجمالي ضريبة المخرجات (البند 6) ناقص إجمالي ضريبة المدخلات (البند 12)', 'Total Output VAT (Item 6) minus Total Input VAT (Item 12)')}</p>
 </div>
 ...
 <span className="text-sm text-slate-500 ml-2 font-medium">{_t('ريال', 'SAR')}</span>
 </div>
 </div>
 </div>
 </div>
