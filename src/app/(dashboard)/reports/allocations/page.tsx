...
export default function AllocationsDashboard() {
 ...
 <div>
 <h1 className="text-3xl font-bold text-slate-800">{_t('محرك التخصيص', 'Allocation Engine')}</h1>
 <p className="text-sm text-slate-500 mt-1">{_t('توزيع التكاليف والإيرادات المتتالية متعددة المستويات', 'Multi-level Cascading Cost & Revenue Allocations')}</p>
 </div>
 <button className="px-4 py-2 bg-indigo-600 text-white rounded-md shadow-sm hover:bg-indigo-700">
 {_t('+ جديد تخصيص قاعدة', '+ New Allocation Rule')}</button>
 </div>

 <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
 {/* Rules List */}
 <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm border border-slate-100 h-[600px] overflow-y-auto">
 <h3 className="text-lg font-bold text-slate-800 mb-4">{_t('القواعد النشطة', 'Active Rules')}</h3>
 
 <div className="space-y-4">
 <div className="p-4 border border-indigo-100 bg-indigo-50 rounded-lg cursor-pointer">
 <div className="flex justify-between items-start">
 <div>
 <h4 className="font-semibold text-indigo-900">{_t('تخصيص دعم تكنولوجيا المعلومات', 'IT Support Allocation')}</h4>
 <p className="text-xs text-indigo-700 mt-1">Source: CC-900 (IT Dept)</p>
 </div>
 <span className="bg-indigo-200 text-indigo-800 text-[10px] px-2 py-1 rounded-full font-bold">{_t('الأولوية 1', 'Priority 1')}</span>
 </div>
 <div className="mt-3 flex justify-between items-center text-sm">
 <span className="text-indigo-600 font-medium">Basis: Headcount</span>
 <button onClick={handleSimulate} className="text-xs bg-white px-2 py-1 rounded text-indigo-600 font-medium hover:bg-indigo-100">{_t('محاكاة', 'Simulate')}</button>
 </div>
 </div>
 ...
 <div className="flex justify-between items-start">
 <div>
 <h4 className="font-semibold text-slate-800">{_t('تقسيم إيجار المنشأة', 'Facility Rent Split')}</h4>
 <p className="text-xs text-slate-500 mt-1">Source: CC-800 (HQ)</p>
 </div>
 <span className="bg-slate-100 text-slate-600 text-[10px] px-2 py-1 rounded-full font-bold">{_t('الأولوية 2', 'Priority 2')}</span>
 </div>
 <div className="mt-3 flex justify-between items-center text-sm">
 <span className="text-slate-500 font-medium">Basis: SQFT</span>
 <button className="text-xs bg-slate-50 px-2 py-1 rounded text-slate-600 font-medium hover:bg-slate-200">{_t('محاكاة', 'Simulate')}</button>
 </div>
 </div>
 </div>
 </div>
 ...
 <h3 className="text-lg font-bold text-slate-800 mb-4">{_t('معاينة المحاكاة', 'Simulation Preview')}</h3>
 
 {!simulationData ? (
 <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
 <svg className="w-16 h-16 mb-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
 <p>{_t('حدد قاعدة وانقر فوق محاكاة لمعاينة مصفوفة التخصيص.', 'Select a rule and click Simulate to preview the allocation matrix.')}</p>
 </div>
 ) : (
 ...
 <span className="block text-xs text-slate-500 mb-1">{_t('أساس التخصيص', 'Allocation Basis')}</span>
 ...
 <th className="py-3 px-4 font-medium text-slate-500 text-sm">{_t('مركز التكلفة المستهدفة', 'Target Cost Center')}</th>
 <th className="py-3 px-4 font-medium text-slate-500 text-sm">{_t('وزن / %', 'Weight / %')}</th>
 <th className="py-3 px-4 font-medium text-slate-500 text-sm">{_t('المبلغ المخصص', 'Allocated Amount')}</th>
 </tr>
 </thead>
 <tbody>
 ...
 <td className="py-3 px-4 text-sm text-slate-800">{_t('إجمالي المخصص', 'Total Allocated')}</td>
 ...
 <button className="px-4 py-2 bg-indigo-600 text-white rounded-md shadow-sm hover:bg-indigo-700 font-medium">{_t('تنفيذ ونشر اليومية', 'Execute & Post Journal')}</button>
 </div>
 </div>
 )}
 </div>
