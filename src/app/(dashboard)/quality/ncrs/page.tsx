            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-gray-900 flex items-center gap-2">
                        <FileWarning className="w-8 h-8 text-red-600" />{_t('تقارير عدم المطابقة (NCR)', 'Non-Conformance Reports (NCR)')}</h1>
                    <p className="text-gray-500 mt-1">{_t('إدارة قرارات وتصرفات مجلس مراجعة المواد (MRB).', 'Manage material review board (MRB) decisions and dispositions.')}</p>
                </div>
                <div className="flex gap-2">
                    ...
                    <Button className="bg-red-600 hover:bg-red-700 text-white shadow-sm">
                        <Plus className="w-4 h-4 mr-2" />{_t('إنشاء إن سي آر', 'Create NCR')}</Button>
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 text-gray-600 border-b">
                            <tr>
                                <th className="px-4 py-3 font-medium">{_t('معرف NCR', 'NCR ID')}</th>
                                <th className="px-4 py-3 font-medium">{_t('التاريخ', 'Date')}</th>
                                <th className="px-4 py-3 font-medium">{_t('مرجع التفتيش', 'Inspection Ref')}</th>
                                <th className="px-4 py-3 font-medium">{_t('خطورة', 'Severity')}</th>
                            ...
                            {ncrs.map((ncr) => (
                                <tr key={ncr.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50">{_t('مراجعة MRB', 'MRB Review')}<ArrowRight className="w-4 h-4 ml-1" />
                                        </Button>
                                    </td>
                                </tr>
                            ))}
                                ...
                                <tr>
                                    <td colSpan={7} className="px-4 py-12 text-center text-gray-500">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-lg font-medium text-gray-900">{_t('لم يتم العثور على بطاقات NCR', 'No NCRs Found')}</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
