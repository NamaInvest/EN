...
export default function LeaseContractsPage() {
 ...
 {loading ? (
 <div className="p-8 text-center text-slate-500">جاري تحميل العقود...</div>
 ) : leases.length === 0 ? (
 <div className="p-8 text-center text-slate-500">لا توجد عقود مسجلة.</div>
 ) : (
 <table className="w-full text-left border-collapse">
 <thead>
 <tr className="bg-slate-50 border-b border-slate-200 text-slate-600">
 <th className="p-4 font-semibold">رقم العقد</th>
 <th className="p-4 font-semibold">المستأجر</th>
 <th className="p-4 font-semibold">الوحدة</th>
 <th className="p-4 font-semibold">الفترة</th>
 <th className="p-4 font-semibold text-right">قيمة الإيجار</th>
 ...
 <td className="p-4">{lease.tenant?.name || 'غير معروف'}</td>
 <td className="p-4 text-sm text-slate-600">
 {lease.unit?.property?.name} - وحدة {lease.unit?.unitNumber}
 </td>
 <td className="p-4 text-sm text-slate-500">
 {new Date(lease.startDate).toLocaleDateString('en-GB')} - <br/>{new Date(lease.endDate).toLocaleDateString('en-GB')}
 </td>
 <td className="p-4 font-medium text-right bg-slate-50/50">
 {lease.rentAmount.toLocaleString()} ر.س <br/><span className="text-xs text-slate-400">{lease.paymentFrequency}</span>
 </td>
 <td className="p-4">
