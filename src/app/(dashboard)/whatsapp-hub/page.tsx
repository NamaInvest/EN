...
export default function WhatsAppHubPage() {
  ...
  const fetchSessions = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // silently fail for polling to not spam toast
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBroadcast = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toastSuccess(`✅ اكتملت الحملة! تم الإرسال بنجاح إلى: ${data.stats.success} عميل، فشل: ${data.stats.failed}`);
        setBroadcastMessage('');
      } else {
        toastError(t('sys.str_1526') + (data.error || t('sys.str_1527')));
      }
    } catch (err) {
      toastError(t('sys.str_1528'));
    } finally {
      setSending(false);
    }
  };
          ...
          <div style={{ flex: 1, padding: '40px', overflowY: 'auto' }}>
            <div style={{ maxWidth: '600px', margin: '0 auto', background: '#fff', padding: '30px', borderRadius: '16px', boxShadow: 'var(--shadow-md)', border: '1px solid var(--border)' }}>
              ...
              <button 
                ...
                disabled={sending}
                style={{ width: '100%', padding: '16px', background: 'var(--primary)', color: '#fff', border: 'none', borderRadius: '12px', fontSize: '16px', fontWeight: 'bold', cursor: sending ? 'not-allowed' : 'pointer', transition: 'transform 0.1s', opacity: sending ? 0.7 : 1 }}
                onMouseDown={e => { if(!sending) e.currentTarget.style.transform = 'scale(0.98)' }}
                onMouseUp={e => { if(!sending) e.currentTarget.style.transform = 'scale(1)' }}
              >
                {sending ? t('sys.str_1525') : t('sys.str_1520')}
              </button>
            </div>
          </div>
        )}
