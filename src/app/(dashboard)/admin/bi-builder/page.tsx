...
export default function BIBuilderDashboard() {
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto space-y-6">
            ...
            <p className="text-gray-500">Design your own data reports dynamically and export them to CSV.</p>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {/* Configuration Panel */}
                <div className="col-span-1 bg-white p-6 rounded-xl shadow-sm border space-y-6">
                    <h2 className="font-semibold flex items-center gap-2"><Settings className="w-4 h-4"/> Data Source</h2>
                    
                    <div className="space-y-2">
                        <label className="text-sm text-gray-600">Primary Table</label>
                        <select 
                            className="w-full border p-2 rounded" 
                            value={selectedModel} 
                            onChange={(e) => {
                                setSelectedModel(e.target.value);
                                setSelectedFields([]); // reset fields on model change
                            }}
                        >
                    ...
                    <div className="space-y-2">
                        <label className="text-sm text-gray-600">Row Limit</label>
                        <input 
                            type="number" 
                            value={limit} 
                ...
                <div className="col-span-3 bg-white p-6 rounded-xl shadow-sm border flex flex-col">
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="font-semibold flex items-center gap-2"><TableIcon className="w-5 h-5"/> Report Preview</h2>
                    ...
                    <div className="flex-1 overflow-auto border rounded bg-gray-50 min-h-[400px]">
                        {results.length === 0 ? (
                            <div className="flex items-center justify-center h-full text-gray-400">
                                {loading ? 'Running query...' : 'No data. Select columns and click Run.'}
                            </div>
                        ) : (
