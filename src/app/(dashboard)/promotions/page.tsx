            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-gray-900 flex items-center gap-2">
                        <Tag className="w-8 h-8 text-rose-600" />{_t('محرك الترقيات', 'Promotions Engine')}</h1>
                    <p className="text-gray-500 mt-1">{_t('إدارة الخصومات وعروض BOGO والكوبونات والتكديس الترويجي.', 'Manage discounts, BOGO offers, coupons, and promotional stacking.')}</p>
                </div>
                <div className="flex gap-2">
                    <Button variant="outline" className="bg-white">{_t('عرض القسائم', 'View Coupons')}</Button>
                    <Button className="bg-rose-600 hover:bg-rose-700 text-white shadow-sm">
                        <Plus className="w-4 h-4 mr-2" />{_t('إنشاء الترويج', 'Create Promotion')}</Button>
                </div>
            </div>
                ...
                <Card className="bg-gradient-to-br from-rose-50 to-white border-rose-100">
                    <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                            <p className="text-sm font-medium text-rose-600">{_t('الحملات النشطة', 'Active Campaigns')}</p>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 text-gray-600 border-b">
                            <tr>
                                <th className="px-4 py-3 font-medium">{_t('اسم الترويج', 'Promotion Name')}</th>
                                <th className="px-4 py-3 font-medium">{_t('النوع', 'Type')}</th>
                                <th className="px-4 py-3 font-medium">{_t('بلح', 'Dates')}</th>
                            ...
                            {promotions.map((promo) => {
                                ...
                                <tr key={promo.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        <Button variant="ghost" size="sm" className="text-rose-600 hover:text-rose-700 hover:bg-rose-50">{_t('يدير', 'Manage')}</Button>
                                    </td>
                                </tr>
                            )})}
                            {promotions.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="px-4 py-12 text-center text-gray-500">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-lg font-medium text-gray-900">{_t('لا توجد عروض ترويجية نشطة', 'No Promotions Active')}</p>
                                            <p className="text-sm mt-1">{_t('عزز مبيعاتك عن طريق إنشاء منطق خصم مخصص.', 'Boost your sales by creating custom discount logic.')}</p>
