    ...
    {showModal && (
      <div className="modal-backdrop" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 9999 }}>
        <div className="modal bg-white" style={{ maxWidth: '1000px', width: '95%', borderRadius: '12px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', maxHeight: '90vh', overflowY: 'auto', margin: 0 }}>
          ...
          <form onSubmit={handleSubmit(handleCreateOrder)} style={{ padding: '20px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '20px' }}>
              <div className="input-group" style={{ margin: 0 }}>
                <label className="input-label">{t('sys.str_953')}</label>
                <select className={`input ${errors.supplierId ? 'border-red-500' : ''}`} {...register('supplierId')}>
            ...
            <table className="table" style={{ marginBottom: '15px' }}>
              ...
              <tbody>
                {fields.map((field, idx) => (
                  <tr key={field.id}>
                    <td>
                      <input className={`input ${errors.items?.[idx]?.productName ? 'border-red-500' : ''}`} {...register(`items.${idx}.productName` as const)} />
                    </td>
                    <td>
                      <input type="number" step="0.01" className={`input ${errors.items?.[idx]?.quantity ? 'border-red-500' : ''}`} {...register(`items.${idx}.quantity` as const, { valueAsNumber: true })} />
                    </td>
                    <td>
                      <input type="number" step="0.01" className={`input ${errors.items?.[idx]?.price ? 'border-red-500' : ''}`} {...register(`items.${idx}.price` as const, { valueAsNumber: true })} />
                    </td>
                    <td>
                      <button type="button" onClick={() => remove(idx)} className="btn btn-ghost" style={{ color: 'red' }}>🗑️</button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
            <button type="button" onClick={() => append({ productId: '1', productName: '', quantity: 1, price: 0 })} className="btn btn-outline" style={{ marginBottom: '20px' }}>{t('sys.str_959')}</button>
