        ...
        <div className="max-w-7xl mx-auto space-y-6 p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-gray-900 flex items-center gap-2">
                        <CreditCard className="w-8 h-8 text-teal-600" />{_t('بطاقات الهدايا', 'Gift Cards')}</h1>
                    <p className="text-gray-500 mt-1">{_t('إصدار وإدارة وتتبع بطاقات الهدايا الرقمية والمادية.', 'Issue, manage, and track digital and physical gift cards.')}</p>
                </div>
                <div className="flex gap-2">
                    <Button className="bg-teal-600 hover:bg-teal-700 text-white shadow-sm">
                        <Plus className="w-4 h-4 mr-2" />{_t('إصدار بطاقة الهدايا', 'Issue Gift Card')}</Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="bg-gradient-to-br from-teal-50 to-white border-teal-100">
                    <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                            <p className="text-sm font-medium text-teal-600">{_t('البطاقات النشطة', 'Active Cards')}</p>
                ...
                <Card>
                    <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                            <p className="text-sm font-medium text-gray-500">{_t('المسؤولية الكاملة', 'Total Liability')}</p>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 text-gray-600 border-b">
                            <tr>
                                ...
                                <th className="px-4 py-3 font-medium">{_t('الرصيد الأولي', 'Initial Balance')}</th>
                                <th className="px-4 py-3 font-medium">{_t('الرصيد الحالي', 'Current Balance')}</th>
                                <th className="px-4 py-3 font-medium">{_t('صدرت في', 'Issued At')}</th>
                            ...
                            {giftCards.map((card) => (
                                <tr key={card.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        ...
                                        <Button variant="ghost" size="sm" className="text-teal-600 hover:text-teal-700 hover:bg-teal-50 ml-1">{_t('يدير', 'Manage')}</Button>
                                    </td>
                                </tr>
                            ))}
                            {giftCards.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="px-4 py-12 text-center text-gray-500">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-lg font-medium text-gray-900">{_t('لم يتم إصدار أي بطاقات هدايا', 'No Gift Cards Issued')}</p>
                                            <Button className="mt-4 bg-teal-600 hover:bg-teal-700 text-white">
                                                <Plus className="w-4 h-4 mr-2" />{_t('إصدار البطاقة الأولى', 'Issue First Card')}</Button>
                                        </div>
                                    </td>
                                </tr>
                            )}
