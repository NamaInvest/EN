...
export default function EditQuotationPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const [cRes, pRes, qRes] = await Promise.all([
        ...
        fetch(`/api/sales/quotations/${quoteId}`, { headers: { Authorization: `Bearer ${token}` } }),
      ]);
      ...
      if (qRes.ok) {
        ...
        if (qData.status !== 'DRAFT') {
          toastError('لا يمكن تعديل عرض السعر إلا إذا كان في حالة مسودة');
          router.push(`/sales/quotations/${quoteId}`);
          return;
        }

        setCustomerId(qData.customerId || '');
        setContactName(qData.contactName || '');
        setContactEmail(qData.contactEmail || '');
        setContactPhone(qData.contactPhone || '');
        setValidUntil(qData.validUntil ? qData.validUntil.split('T')[0] : '');
        setCurrency(qData.currency || 'SAR');
        setTerms(qData.terms || '');
        setNotes(qData.notes || '');
        setLines(
          qData.lines.map((l: any) => ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        toastSuccess('تم تحديث عرض السعر بنجاح');
        router.push(`/sales/quotations/${quoteId}`);
      } else {
        const err = await res.json();
        toastError(err.error || 'فشل تحديث عرض السعر');
      }
    } catch (e) {
  ...
  if (loading) {
    return <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>جاري تحميل البيانات...</div>;
  }

  return (
    <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '20px' }}>
      <div className="page-header" style={{ marginBottom: '20px' }}>
        <h1 className="page-title" style={{ fontSize: '24px', fontWeight: 'bold' }}>تعديل عرض السعر</h1>
        <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '5px' }}>
          أنت الآن تقوم بتعديل المسودة الخاصة بعرض السعر.
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div className="card" style={{ padding: '20px', backgroundColor: 'var(--card-bg, white)', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '15px' }}>
            ...
            <div className="input-group" style={{ margin: 0 }}>
              ...
              <input type="text" className="input" value={contactPhone} onChange={(e) => setContactPhone(e.target.value)} />
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '20px' }}>
          ...
          <div className="card" style={{ padding: '20px', backgroundColor: 'var(--card-bg, white)', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
            ...
            <div className="input-group" style={{ margin: 0 }}>
              <label className="input-label">شروط وأحكام العرض</label>
              <textarea
                className="input"
                value={terms}
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <strong>{subtotal.toLocaleString(undefined, { minimumFractionDigits: 2 })} {currency}</strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#ef4444' }}>
                  <span style={{ color: 'var(--text-muted)' }}>إجمالي الخصم:</span>
                  <strong>-{discountTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })} {currency}</strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-muted)' }}>ضريبة القيمة المضافة:</span>
                  <strong>{taxTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })} {currency}</strong>
                </div>
              </div>
            </div>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '10px' }}>
          <Link href={`/sales/quotations/${quoteId}`}>
