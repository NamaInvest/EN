...
export default function ChecksPage() {
    ...
    try {
      ...
      const res = await fetch('/api/finance/checks', {
        ...
        body: JSON.stringify({
          ...
          notes: data.notes,
          type: tab,
          customerId: tab === 'RECEIVABLE' ? parseInt(data.partyId) : null,
          supplierId: tab === 'PAYABLE' ? parseInt(data.partyId) : null,
        })
      });
      if (res.ok) {
        setShowModal(false);
        reset();
        loadChecks();
        toastSuccess('تم حفظ الشيك بنجاح');
      } else {
        toastError(t('sys.str_2741'));
      }
    } catch (e) {
  ...
  return (<>
    ...
    <div className="tabs" style={{ display: 'flex', gap: '10px', padding: '0 20px', marginBottom: '15px' }}>
      <button onClick={() => { setTab('RECEIVABLE'); reset(); }} className={`btn ${tab === 'RECEIVABLE' ? 'btn-primary' : 'btn-outline'}`}>{t('sys.str_2724')}</button>
      <button onClick={() => { setTab('PAYABLE'); reset(); }} className={`btn ${tab === 'PAYABLE' ? 'btn-primary' : 'btn-outline'}`}>{t('sys.str_2725')}</button>
    </div>
        ...
        <div className="modal" style={{ maxWidth: '600px', width: '95%', backgroundColor: 'var(--card-bg, white)', borderRadius: '12px', padding: '24px', position: 'relative' }}>
          ...
          <form onSubmit={handleSubmit(handleCreate)} style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginTop: '15px' }}>
            <div className="input-group" style={{ margin: 0 }}>
              ...
              <input className={`input ${errors.checkNumber ? 'border-red-500' : ''}`} {...register('checkNumber')} />
              {errors.checkNumber && <span className="text-red-500 text-xs mt-1">{errors.checkNumber.message}</span>}
            </div>
            <div className="input-group" style={{ margin: 0 }}>
              <label className="input-label">{t('sys.str_2737')}</label>
              <input className={`input ${errors.bankName ? 'border-red-500' : ''}`} {...register('bankName')} />
              {errors.bankName && <span className="text-red-500 text-xs mt-1">{errors.bankName.message}</span>}
            </div>
            <div className="input-group" style={{ margin: 0 }}>
              <label className="input-label">{t('sys.str_666')}</label>
              <input type="date" className={`input ${errors.dueDate ? 'border-red-500' : ''}`} {...register('dueDate')} />
            ...
            <div className="input-group" style={{ margin: 0 }}>
              ...
              <input type="number" step="0.01" className={`input ${errors.amount ? 'border-red-500' : ''}`} {...register('amount', { valueAsNumber: true })} />
            ...
            <div className="input-group" style={{ margin: 0 }}>
              ...
              <select className={`input ${errors.partyId ? 'border-red-500' : ''}`} {...register('partyId')}>
                <option value="">{t('sys.str_2739')}</option>
                {parties.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
              </select>
              {errors.partyId && <span className="text-red-500 text-xs mt-1">{errors.partyId.message}</span>}
            </div>
            <div className="input-group" style={{ margin: 0 }}>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '10px' }}>
              ...
              <button type="submit" disabled={saving} className="btn btn-primary">{saving ? t('sys.str_454') : t('sys.str_2740')}</button>
            </div>
          </form>
        </div>
      </div>
