...
export default function BankReconciliation() {
 ...
 <RefreshCw className="w-8 h-8 mr-3 text-indigo-600" />{_t('تسوية البنك', 'Bank Reconciliation')}</h1>
 ...
 <option>{_t('مصرف الراجحي (ريال سعودي) - STMT-2026-05', 'Al Rajhi Bank (SAR) - STMT-2026-05')}</option>
 <option>{_t('البنك الوطني السويسري (USD) - STMT-2026-05', 'SNB (USD) - STMT-2026-05')}</option>
 </select>
 ...
 <UploadCloud className="w-4 h-4 mr-2 text-indigo-600" />{_t('استيراد MT940', 'Import MT940')}</button>
 ...
 <RefreshCw className="w-4 h-4 mr-2" />{_t('تشغيل المطابقة التلقائية', 'Run Auto-Match')}</button>
 </div>
 </div>

 <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 shrink-0">
 <div className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm text-center">
 <p className="text-sm font-medium text-slate-500">{_t('رصيد البيان', 'Statement Balance')}</p>
 <h3 className="text-2xl font-bold text-slate-900 font-mono mt-1">{Number(summary.statementBalance).toLocaleString()}</h3>
 </div>
 <div className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm text-center">
 <p className="text-sm font-medium text-slate-500">{_t('رصيد كتاب GL', 'GL Book Balance')}</p>
 ...
 <p className="text-sm font-medium text-yellow-800 dark:text-yellow-400">{_t('الفرق للحل', 'Difference to Resolve')}</p>
 ...
 <p className="text-sm font-medium text-indigo-800 dark:text-indigo-400">{_t('معدل المباراة', 'Match Rate')}</p>
 ...
 <FileText className="w-4 h-4 mr-2" />{_t('خطوط كشف الحساب البنكي', 'Bank Statement Lines')}</h3>
 ...
 {loading && <p className="text-center p-4 text-slate-500">{_t('جارٍ تحميل البيانات...', 'Loading statements...')}</p>}
 ...
 <span className="text-xs px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100 font-medium">{_t('لا مثيل له (انقر للمطابقة)', 'UNMATCHED (Click to Match)')}</span>
 </div>
 </div>
 ))}
 ...
 <div className="p-3 border-b border-slate-200 flex justify-between items-center bg-slate-50 ">
 <h3 className="text-sm font-bold text-slate-700 uppercase flex items-center">
 <CheckCircle className="w-4 h-4 mr-2" />{_t('مطابقة المرشحين (GL)', 'Match Candidates (GL)')}</h3>
 </div>
 ...
 <Link className="w-4 h-4 mr-2" />{_t('مطابقة المحدد', 'Match Selected')}</button>
 <button className="w-full py-2 mt-2 bg-white border border-slate-300 hover:bg-slate-50 dark:hover:bg-gray-700 text-slate-700 font-medium rounded-md">{_t('البحث يدويا', 'Search Manually')}</button>
 </div>
 </div>
 </div>
