  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6 flex items-center gap-2">
        <Layers className="text-primary" /> {_t('شاشة مراقبة موجات WMS', 'WMS Waves Monitor')}
      </h1>
      ...
      {loading ? (
        ...
        <div className="card p-8 text-center text-red-500 border border-red-200 bg-red-50 rounded-xl shadow-sm">
          ...
          <p>{_t('فشل تحميل موجات WMS.', 'Failed to load WMS waves.')} {data?.error}</p>
        </div>
      ) : data.length === 0 ? (
        <div className="card p-8 text-center border border-(--border) bg-(--bg-secondary) rounded-xl shadow-sm">
          <Search className="mx-auto mb-4 text-gray-400" size={48} />
          <p className="text-(--text-muted)">{_t('لم يتم العثور على أي موجات التقاط نشطة.', 'No active picking waves found.')}</p>
        </div>
      ) : (
        <div className="overflow-x-auto card border border-(--border) rounded-xl shadow-sm">
          <table className="w-full text-left border-collapse" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            <thead className="bg-(--bg-secondary) border-b border-(--border)">
              <tr>
                <th className="p-4 font-semibold text-sm">{_t('معرف الموجة', 'Wave ID')}</th>
                <th className="p-4 font-semibold text-sm">{_t('المستودع', 'Warehouse')}</th>
