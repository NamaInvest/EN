        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">سجل المنشأ اللامركزي (Blockchain Traceability)</h1>
                        <p className="text-gray-500 font-mono text-sm mt-1">Network: {ledger?.network || 'Loading...'} | Consensus: {ledger?.consensus || '...'}</p>
                    </div>
                </div>
            ...
            <div className="space-y-4">
                {loading ? <div className="text-center py-10">جاري مزامنة دفتر الأستاذ الموزع...</div> :
                 ledger?.blocks?.length === 0 ? <div className="text-center py-10 text-gray-500">لا توجد كتل مسجلة</div> :
                 ledger?.blocks?.map((block: any, i: number) => (
                    <div key={block.blockId} className="bg-slate-900 text-slate-300 rounded-2xl p-6 shadow-xl border border-slate-700 relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-4 relative z-10">
                            ...
                            {block.isValid && <div className="flex items-center gap-1 text-emerald-400 bg-emerald-400/10 px-3 py-1 rounded-full text-xs font-bold"><ShieldCheck className="w-4 h-4"/> Verified</div>}
                        </div>
                        ...
                        <div className="space-y-2 relative z-10">
                            <div className="bg-black/50 p-3 rounded-lg border border-slate-800">
                                <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Previous Hash</p>
                            ...
                            <div className="bg-emerald-900/20 p-3 rounded-lg border border-emerald-500/30">
                                <p className="text-[10px] text-emerald-500/70 uppercase tracking-wider mb-1">Block Hash (SHA-256)</p>
