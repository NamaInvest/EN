        ...
        <div className="max-w-6xl mx-auto space-y-6 p-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-gray-500 mt-2">{_t('قم بتكوين سير العمل الآلي وسلاسل الموافقة ومشغلات EventBus.', 'Configure automated workflows, approval chains, and EventBus triggers.')}</p>
                </div>
                <Button className="flex items-center gap-2">
                    <Plus className="w-4 h-4" />{_t('قاعدة جديدة', 'New Rule')}</Button>
            </div>
                ...
                {rules.map(rule => (
                    <Card key={rule.id} className="p-4 flex items-center justify-between border-l-4 border-l-indigo-500">
                        ...
                        <div className="flex gap-2">
                            ...
                            <Button variant="destructive" size="sm">{_t('إبطال', 'Disable')}</Button>
                        </div>
                    </Card>
                ))}
            </div>
            ...
            <div className="mt-8 p-6 bg-slate-900 rounded-lg text-white">
                <h3 className="text-xl font-bold mb-4 flex items-center gap-2"><ServerCrash className="w-5 h-5"/>{_t('مراقب الأحداث الحية', 'EventBus Live Monitor')}</h3>
                <div className="h-48 overflow-y-auto font-mono text-sm space-y-2">
                    ...
                    <p className="animate-pulse text-indigo-400">{_t('إستماع للأحداث الجديدة...', 'Listening for new events...')}</p>
                </div>
            </div>
        </div>
    );
