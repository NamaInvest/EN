        ...
        <div className="min-h-screen p-6" dir="rtl">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-[var(--text-muted)] text-sm mt-1">تتبع الدفعات والصلاحيات — الأول انتهاءً هو الأول صرفاً</p>
                </div>
                            ...
                            {filtered.map(b => (
                                <tr key={b.id} className={`border-b border-[var(--border-light)] hover:bg-[var(--bg-card-hover)] ${STATUS_CONFIG[b.status]?.row}`}>
                                    ...
                                    <td className="p-3">
                                        ...
                                        {b.status === 'ok' && <button className="text-xs px-2 py-1 bg-gray-800 text-[var(--text-muted)] rounded hover:bg-gray-700">📋 عرض</button>}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
