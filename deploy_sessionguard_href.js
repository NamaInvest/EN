...
txt = txt.replace(/if \\(!token\\) \\{ router\\.replace\\('\\/login'\\); return; \\}/g, "if (!token) { window.location.href = '/login'; return; }");

// And also replace the other one inside the session_replaced if block for maximum safety
txt = txt.replace(/router\\.replace\\('\\/login'\\);/g, "window.location.href = '/login';");
