...
export async function POST(req: NextRequest) {
    try {
        ...
        const { customerId, frequency, startDate, items, subtotal, taxValue, total, notes } = body;
        ...
        const maxOrder = await prisma.salesOrder.aggregate({ _max: { orderNo: true } });
...
export async function GET(req: NextRequest) {
    try {
        ...
        const formatted = contracts.map(c => {
            let meta = { nextBillingDate: c.date, frequency: c.status.replace('RECURRING_', ''), extraNotes: '' };
            try { if (c.notes) meta = JSON.parse(c.notes); } catch(e){}
