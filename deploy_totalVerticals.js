...
conn.on('ready', () => {
    console.log('--- TRANSMITTING TOTAL VERTICAL DOMINATION TO FLEET ---');
    ...
    conn.sftp((err, sftp) => {
        ...
        // Push to n1 and namainvist explicitly to override the old file
        sftp.fastPut(localPath, '/www/wwwroot/n1.namainvist.com/src/app/layout.tsx', (err1) => {
            ...
            sftp.fastPut(localPath, '/www/wwwroot/namainvist.com/src/app/layout.tsx', (err2) => {
                if(err2) throw err2;
                console.log("✅ Core Layout patched with 12 Vertical AI Directives!");
                
                const bashScript = `
#!/bin/bash
# Sync layout rapidly across the background nodes
...
echo "🚀 Rebuilding Root Domain..."
cd /www/wwwroot/namainvist.com
npm run build
pm2 reload all

echo "🚀 Background Rebuilding Subnodes N1-N10..."
nohup bash -c '
for i in {1..10}; do
...
' > /tmp/fleet_12_vertical_build.log 2>&1 &
                `;
                ...
                conn.exec(bashScript, (execErr, stream) => {
                    ...
                    stream.on('close', () => {
                        console.log('✅ TOTAL VERTICAL DEPLOYMENT COMPLETE.');
                        conn.end();
                    });
                });
            });
